...
extern int double_tap_enable;					//SW4-HL-Touch-EAG-HX8527-DoubleTap-00+_20180927

int incell_double_tap_enable = 0;	//SW4-HL-Touch-ImplementVirtualFileNode-00+_20180627

extern int gdouble_tap_enable_nvt;		//SW4-HL-Touch-RHD-NT36525-DoubleTap-00+_20180920
...
//FIHTDC - gatycclu - OC6 - Show panel tp data{
static char tp_data_reg[2] = {0xC6, 0x00};
void fih_get_panel_tp_data(struct mdss_dsi_ctrl_pdata *ctrl_pdata, char* tp_data)
{
  char *rx_buf;

  rx_buf = kzalloc(PANEL_REG_ADDR_LEN, GFP_KERNEL);
  mdss_dsi_panel_cmd_read(ctrl_pdata, tp_data_reg[0], tp_data_reg[1],
                                          NULL, rx_buf, 3);
  pr_info("%s: LCM E8h = 0x%x,0x%x, 0x%x\n", __func__, rx_buf[0], rx_buf[1], rx_buf[2]);
  sprintf(tp_data, "0x%x\n",rx_buf[2]);
  kfree(rx_buf);
}
//FIHTDC - gatycclu - OC6 - Show panel tp data}
...
static int mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	{
		case FIH_HX83102_HD_PLUS_VIDEO_PANEL:
		case FIH_HX83102_HD_PLUS_INNO_VIDEO_PANEL:
			{
				pr_debug("[HL]%s, %d: FIH_HX83102_HD_PLUS_VIDEO_PANEL or FIH_HX83102_HD_PLUS_INNO_VIDEO_PANEL\n", __func__, __LINE__);

				pr_debug("[HL]%s, %d: incell_double_tap_enable = %d\n", __func__, __LINE__, incell_double_tap_enable);
				if (incell_double_tap_enable)	//Double Tap Option is On, NO PULL POWERS & RESET PINS
				{
					//DO NOTHING
					pr_debug("[HL]%s, %d: FORCE NOOOOOOOOOOOOOOOOOOOOO PULL POWERS & RESET PINS\n", __func__, __LINE__);
				}
				else	//Doble Tap Option is Off, PULL POWERS & RESET PINS
				{
					pr_debug("[HL]%s, %d: mdss_dsi_panel_reset(pdata, 0) <-- START\n", __func__, __LINE__);
					ret = mdss_dsi_panel_reset(pdata, 0);
					pr_debug("[HL]%s, %d: mdss_dsi_panel_reset(pdata, 0) <-- END\n", __func__, __LINE__);
					if (ret) {
					...
					ret = msm_mdss_enable_vreg(
						...
						ctrl_pdata->panel_power_data.num_vreg, 0);
									pr_debug("[HL]%s, %d: msm_mdss_enable_vreg(0) <-- END\n", __func__, __LINE__);
									if (ret)
		...
		case FIH_NT36525_HD_PLUS_INNO_VIDEO_PANEL:
			{
				...
				if (gdouble_tap_enable_nvt)	//Double Tap Option is On, NO PULL POWERS & RESET PINS
				{
					//DO NOTHING
					pr_debug("[HL]%s, %d: FORCE NOOOOOOOOOOOOOOOOOOOOO PULL POWERS & RESET PINS\n", __func__, __LINE__);
				}
			...
			break;
		case FIH_JD9365Z_CTC_HD_PLUS_VIDEO_PANEL:
			{
				pr_debug("[HL]%s, %d: FIH_JD9365Z_CTC_HD_PLUS_VIDEO_PANEL\n", __func__, __LINE__);

				pr_debug("[HL]%s, %d: double_tap_enable = %d\n", __func__, __LINE__, double_tap_enable);
				if (double_tap_enable)	//Double Tap Option is On, NO PULL POWERS & RESET PINS	//SW4-HL-Touch-EAG-HX8527-DoubleTap-00+_20180927
				{
					//DO NOTHING
					pr_debug("[HL]%s, %d: FORCE NOOOOOOOOOOOOOOOOOOOOO PULL POWERS & RESET PINS\n", __func__, __LINE__);
				}
...
//SHP OD6 Panel bringup, 20181107
		...
		case FIH_HX8352_TM_240x400_VIDEO_PANEL:
			{
				pr_err("[HL]%s, %d: FIH_HX8352_TM_240x400_VIDEO_PANEL\n", __func__,  __LINE__);
				...
				if (ret)
				  pr_err("%s: failed to disable vregs for %s\n",
						  __func__, __mdss_dsi_pm_name(DSI_PANEL_PM));
				pr_err("[HL]%s, %d: [LCM VCC(3.0V) LOW & BRG VDDS(1.8V) LOW]\n", __func__,  __LINE__); 
				
				udelay(5 * 1000);
				
				//Pull LOW LCM IOVCC(1.8V) Enable Pin
				pr_err("[HL] %s, ctrl_pdata->lcm_iovcc_gpio = %d	**********************\n\n", __func__, ctrl_pdata->lcm_iovcc_gpio);
				gpio_set_value(ctrl_pdata->lcm_iovcc_gpio, 0);
				gpio_free(ctrl_pdata->lcm_iovcc_gpio);	
				pr_err("[HL]%s, %d: [LCM 1P8 EN GPIO LOW]\n", __func__,  __LINE__); 	
				...
				pr_err("[HL]%s, %d: [BRG 1P2 EN GPIO LOW]\n", __func__,  __LINE__); 				
			}
			break;
		//SW4-HL-Display-TC358762_HX8352-BringUp-00+}_20181219
...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	pr_debug("[HL]%s, %d: msm_mdss_enable_vreg(1) <-- START\n", __func__, __LINE__);
		...
		case FIH_HX83102_HD_PLUS_INNO_VIDEO_PANEL:
			{
				...
				pr_debug("[HL]%s, %d: incell_double_tap_enable = %d\n", __func__, __LINE__, incell_double_tap_enable);
				if (incell_double_tap_enable)	//Double Tap Option is On, NO PULL POWERS
				{
					//DO NOTHING
					pr_debug("[HL]%s, %d: FORCE NOOOOOOOOOOOOOOOOOOOOO PULL POWERS\n", __func__, __LINE__);
				}
		...
		case FIH_NT36525_HD_PLUS_INNO_VIDEO_PANEL:
			{
				...
				pr_debug("[HL]%s, %d: gdouble_tap_enable_nvt = %d\n", __func__, __LINE__, gdouble_tap_enable_nvt);
				if (gdouble_tap_enable_nvt)	//Double Tap Option is On, NO PULL POWERS But PULL RESET PIN LOW -> HIGH
				{
					pr_debug("[HL]%s, %d: Double Tap Option is On, NO PULL POWERS But PULL RESET PIN LOW -> HIGH\n", __func__, __LINE__);
					pr_debug("[HL]%s, %d: gpio_set_value((ctrl_pdata->rst_gpio), 0) <-- START\n", __func__, __LINE__);
					gpio_set_value((ctrl_pdata->rst_gpio), 0);
					pr_debug("[HL]%s, %d: gpio_set_value((ctrl_pdata->rst_gpio), 0) <-- END\n", __func__, __LINE__);

					udelay(5 * 1000);
					pr_debug("[HL]%s, %d: Delay 5ms After RESET Pin Pulled LOW\n", __func__, __LINE__);
		...
		case FIH_JD9365Z_CTC_HD_PLUS_VIDEO_PANEL:
			{
				...
				pr_debug("[HL]%s, %d: double_tap_enable = %d\n", __func__, __LINE__, double_tap_enable);
				if (double_tap_enable)	//Double Tap Option is On, NO PULL POWERS	//SW4-HL-Touch-EAG-HX8527-DoubleTap-00+_20180927
				{
					//DO NOTHING
					pr_debug("[HL]%s, %d: FORCE NOOOOOOOOOOOOOOOOOOOOO PULL POWERS\n", __func__, __LINE__);
				}
		...
		case FIH_HX8352_TM_240x400_VIDEO_PANEL:
			{
				...
				pr_err("[HL] %s, ctrl_pdata->brg_1p2_en_gpio = %d	**********************\n\n", __func__, ctrl_pdata->brg_1p2_en_gpio);
				...
				pr_err("[HL]%s, %d: [BRG 1P2 EN GPIO  HIGH]\n", __func__,  __LINE__); 
				...
				ret = msm_mdss_enable_vreg(
					ctrl_pdata->panel_power_data.vreg_config,
					ctrl_pdata->panel_power_data.num_vreg, 1);
				pr_err("[HL]%s, %d: msm_mdss_enable_vreg(1) <-- END\n", __func__,  __LINE__);
				if (ret) {
					pr_err("%s: failed to enable vregs for %s\n",
						__func__, __mdss_dsi_pm_name(DSI_PANEL_PM));
					return ret;
				}
				pr_err("[HL]%s, %d: [LCM VCC(3.0V) HIGH & BRG VDDS(1.8V) HIGH]\n", __func__,  __LINE__); 
				
				udelay(5 * 1000);

				//Pull HIGH BRG RST GPIO
				pr_err("[HL] %s, ctrl_pdata->brg_rst_gpio = %d	**********************\n\n", __func__, ctrl_pdata->brg_rst_gpio);
				...
				*/
				if (pdata->panel_info.cont_splash_enabled ||
					  !pdata->panel_info.mipi.lp11_init)
				{
					pr_err("[HL]%s, %d: Pull LCM RESET Pin here!\n", __func__,  __LINE__);

					if (mdss_dsi_pinctrl_set_state(ctrl_pdata, true))
						pr_debug("reset enable: pinctrl not enabled\n");

					pr_err("[HL]%s, %d: mdss_dsi_panel_reset(pdata, 1) <-- START\n", __func__,  __LINE__);
					ret = mdss_dsi_panel_reset(pdata, 1);
					pr_err("[HL]%s, %d: mdss_dsi_panel_reset(pdata, 1) <-- END\n", __func__,  __LINE__);
					if (ret)
						pr_err("%s: Panel reset failed. rc=%d\n",
										__func__, ret);
					pr_err("[HL]%s, %d: [RESET]\n", __func__,  __LINE__); 
...
//SW4-HL-Display-TC358762_HX8352-BringUp-00+{_20181219
static u32 tc358762_i2c_write_reg(struct i2c_client *client, u16 reg, u32 val)
{
	int rc;
	u8 buf[6];
...
static u32 tc358762_i2c_read_reg(struct i2c_client *client, u16 reg)
{
	int rc;
	u32 val = 0;
	u8 buf[6];
...
static u32 tc358762_i2c_one_byte_read_reg(struct i2c_client *client, u16 reg)
{
	int rc;
	u32 val = 0;
	u8 buf[6];
...
static int tc358762_bridge_init_sequence(int force_clk_lane_hs)
{
	...
	{
		pr_err("[HL]%s, %d: force_clk_lane_hs = 0, use External Crystal Clock\n", __func__,  __LINE__);
	...
	{
		pr_err("[HL]%s, %d: force_clk_lane_hs = 1, use Internal MIPI DSI Clock\n", __func__,  __LINE__);
		tc358762_i2c_write_reg(toshiba_bridge_client, 0x047C, 0x00000000);
		tc358762_i2c_write_reg(toshiba_bridge_client, 0x0210, 0x00000007);
	...
	//--------------------Read Chip ID for testing by I2C-------------------------//
	do
	{
		val = tc358762_i2c_read_reg(toshiba_bridge_client, 0x04A0);
		pr_err("[LCM][BRIDGE IC-tc358762]%s: Chip ID = 0x%04x\n", __func__, val);
		
		mdelay(50);

		retry++;
	}
	while ((val != 0x6200) && (retry < 3));
	
	if (val == 0x6200)
	{
		pr_err("[BRG][Bridge IC]%s: Bridge IC is initialized success!\n", __func__);
	}
	else
	{
		pr_err("[BRG][TC358762]%s: Bridge IC is initialized fail!\n", __func__);
	}
	...
	tc358762_i2c_write_reg(toshiba_bridge_client, 0x0440, 0x00000110);
	tc358762_i2c_write_reg(toshiba_bridge_client, 0x0410, 0x00000007);
	tc358762_i2c_one_byte_write_reg(toshiba_bridge_client, 0x0504, 0xE2);
		val = tc358762_i2c_one_byte_read_reg(toshiba_bridge_client, 0x0268);
		pr_err("[LCM][HX8352]%s: 0xE2, 0x%02x\n", __func__, val);
		
		mdelay(50);

		retry++;
	}
	while ((val != 0x15) && (retry < 3));
	
	if (val == 0x15)
	{
		pr_err("[LCM][HX8352]%s: Panel is initialized success!\n", __func__);
	}
	else
	{
		pr_err("[LCM][HX8352]%s: Panel is initialized fail!\n", __func__);
	}
			...
			case FIH_HX83102_HD_PLUS_INNO_VIDEO_PANEL:
				{
					...
					pr_debug("[HL]%s, %d: incell_double_tap_enable = %d\n", __func__, __LINE__, incell_double_tap_enable);
					if (incell_double_tap_enable)	//Double Tap Option is On, NO PULL RESET PINS
					{
						//DO NOTHING
						pr_debug("[HL]%s, %d: FORCE NOOOOOOOOOOOOOOOOOOOOO PULL RESET PINS\n", __func__, __LINE__);
					}
					else	//Doble Tap Option is Off, PULL RESET PINS
					{
						if (mdss_dsi_pinctrl_set_state(ctrl_pdata, true))
							pr_debug("reset enable: pinctrl not enabled\n");
						mdss_dsi_panel_reset(pdata, 1);
			...
			case FIH_JD9365Z_CTC_HD_PLUS_VIDEO_PANEL:
				{
					...
					pr_debug("[HL]%s, %d: double_tap_enable = %d\n", __func__, __LINE__, double_tap_enable);
					if (double_tap_enable)	//Double Tap Option is On, NO PULL RESET PINS	//SW4-HL-Touch-EAG-HX8527-DoubleTap-00+_20180927
					{
						//DO NOTHING
						pr_debug("[HL]%s, %d: FORCE NOOOOOOOOOOOOOOOOOOOOO PULL RESET PINS\n", __func__, __LINE__);
					}
...
//FIHTDC - gatycclu - OC6 - Show panel color data{
void fih_get_color_data (char *color_data)
{
  fih_get_panel_color_data(gpdata, color_data);
  return;
}
EXPORT_SYMBOL(fih_get_color_data);
//FIHTDC - gatycclu - OC6 - Show panel color data}

//FIHTDC - gatycclu - OC6 - Show panel tp data}
void fih_get_tp_data (char *tp_data)
{
  fih_get_panel_tp_data(gpdata, tp_data);
  return;
}
EXPORT_SYMBOL(fih_get_tp_data);
//FIHTDC - gatycclu - OC6 - Show panel tp data}
...
static int fih_set_feature(void)
{
	...
	if (!SendCEBeforeInit && CE_enable)
	{
		...
		pr_debug("[HL]%s: SendCEOnlyAfterResume = 1\n", __func__);
	}

	if (!SendCTBeforeInit && CT_enable)
	{
		SendCTOnlyAfterResume = 1;
		pr_debug("[HL]%s: SendCTOnlyAfterResume = 1\n", __func__);
	}

	if (!SendCABCBeforeInit && CABC_enable)
	{
		SendCABCOnlyAfterResume = 1;
		pr_debug("[HL]%s: SendCABCOnlyAfterResume = 1\n", __func__);
	}
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_LINK_READY:
		pr_debug("[HL]%s, %d: MDSS_EVENT_LINK_READY <-- START\n", __func__, __LINE__);
		...
		mdss_dsi_op_mode_config(pdata->panel_info.mipi.mode,
							pdata);
		pr_debug("[HL]%s, %d: MDSS_EVENT_LINK_READY <-- END\n", __func__, __LINE__);
		break;
	case MDSS_EVENT_UNBLANK:
		pr_debug("[HL]%s, %d: MDSS_EVENT_UNBLANK <-- START\n", __func__, __LINE__);
		if (ctrl_pdata->on_cmds.link_state == DSI_LP_MODE)
			rc = mdss_dsi_unblank(pdata);
		pr_debug("[HL]%s, %d: MDSS_EVENT_UNBLANK <-- END\n", __func__, __LINE__);
		break;
	case MDSS_EVENT_POST_PANEL_ON:
		pr_debug("[HL]%s, %d: MDSS_EVENT_POST_PANEL_ON <-- START\n", __func__, __LINE__);
		rc = mdss_dsi_post_panel_on(pdata);
		pr_debug("[HL]%s, %d: MDSS_EVENT_POST_PANEL_ON <-- END\n", __func__, __LINE__);
		break;
	case MDSS_EVENT_PANEL_ON:
		pr_debug("[HL]%s, %d: MDSS_EVENT_PANEL_ON <-- START\n", __func__, __LINE__);
		...
		//SW4-HL-Display-ImplementCECTCABC-00+}_20160126
		pr_debug("[HL]%s, %d: MDSS_EVENT_PANEL_ON <-- END\n", __func__, __LINE__);
		break;
	case MDSS_EVENT_BLANK:
		pr_debug("[HL]%s, %d: MDSS_EVENT_BLANK <-- START\n", __func__, __LINE__);
		power_state = (int) (unsigned long) arg;
		if (ctrl_pdata->off_cmds.link_state == DSI_HS_MODE)
			rc = mdss_dsi_blank(pdata, power_state);
		pr_debug("[HL]%s, %d: MDSS_EVENT_BLANK <-- END\n", __func__, __LINE__);
		break;
	case MDSS_EVENT_PANEL_OFF:
		pr_debug("[HL]%s, %d: MDSS_EVENT_PANEL_OFF <-- START\n", __func__, __LINE__);
		power_state = (int) (unsigned long) arg;
		disable_esd_thread();
		ctrl_pdata->ctrl_state &= ~CTRL_STATE_MDP_ACTIVE;
		...
		pr_debug("[HL]%s, %d: MDSS_EVENT_PANEL_OFF <-- END\n", __func__, __LINE__);
		break;
	case MDSS_EVENT_CONT_SPLASH_FINISH:
		pr_debug("[HL]%s, %d: MDSS_EVENT_CONT_SPLASH_FINISH <-- START\n", __func__, __LINE__);
		if (ctrl_pdata->off_cmds.link_state == DSI_LP_MODE)
			rc = mdss_dsi_blank(pdata, MDSS_PANEL_POWER_OFF);
		ctrl_pdata->ctrl_state &= ~CTRL_STATE_MDP_ACTIVE;
		rc = mdss_dsi_cont_splash_on(pdata);
		pr_debug("[HL]%s, %d: MDSS_EVENT_CONT_SPLASH_FINISH <-- END\n", __func__, __LINE__);
		break;
	...
	case MDSS_EVENT_CONT_SPLASH_BEGIN:
		pr_debug("[HL]%s, %d: MDSS_EVENT_CONT_SPLASH_BEGIN <-- START\n", __func__, __LINE__);
		if (ctrl_pdata->off_cmds.link_state == DSI_HS_MODE) {
			/* Panel is Enabled in Bootloader */
			rc = mdss_dsi_blank(pdata, MDSS_PANEL_POWER_OFF);
		}
		pr_debug("[HL]%s, %d: MDSS_EVENT_CONT_SPLASH_BEGIN <-- END\n", __func__, __LINE__);
		break;
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	{
		pr_debug("[HL] %s ---, if (CE_enable && CT_enable && CABC_enable), probe OK, return 0  **********************\n\n", __func__);
		return 0;
	}
	...
	{
		pr_debug("[HL] %s ---, if (SendCEBeforeInit && SendCTBeforeInit && SendCABCBeforeInit), probe OK, return 0  **********************\n\n", __func__);
...
error_pan_node:
	...
	of_node_put(dsi_pan_node);
	pr_debug("[HL] %s ---, error_pan_node, rc = %d  **********************\n\n", __func__, rc);
//SW4-HL-Display-DynamicReadWriteRegister-00+{_20160729
error_no_mem:
	...
	pr_debug("[HL] %s ---, error_no_mem, rc = %d  **********************\n\n", __func__, rc);
//SW4-HL-Display-DynamicReadWriteRegister-00+}_20160729
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	//SW4-HL-Display-EAG_RHD-BringUpLcmDriverIC+{_20180529
	ctrl_pdata->lcm_iovcc_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "fih,lcm-iovcc-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->lcm_iovcc_gpio))
		pr_err("%s:%d, lcm iovcc gpio not specified\n",
						__func__, __LINE__);
	//SW4-HL-Display-EAG_RHD-BringUpLcmDriverIC+}_20180529

	//SW4-HL-Display-OD6-BringUpLcmDriverIC+{_20181107
	ctrl_pdata->tp_reset_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			 "fih,platform-tp-reset-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->tp_reset_gpio))
		pr_err("%s:%d, TP RST gpio not specified\n",
						__func__, __LINE__);
	//SW4-HL-Display-OD6-BringUpLcmDriverIC+}_20181107
