...
//SW4-HL-Display-EnhanceErrorHandling-00*_20170518
static int mdss_dsi_panel_cmds_send(struct mdss_dsi_ctrl_pdata *ctrl,
			struct dsi_panel_cmds *pcmds, u32 flags)
{
	struct dcs_cmd_req cmdreq;
	struct mdss_panel_info *pinfo;
	int len = 1;	//SW4-HL-Display-EnhanceErrorHandling-00*_20170518

	pinfo = &(ctrl->panel_data.panel_info);
	if (pinfo->dcs_cmd_by_left) {
		if (ctrl->ndx != DSI_CTRL_LEFT)
			return len;	//SW4-HL-Display-EnhanceErrorHandling-00*_20170518
	}
	...
	len = mdss_dsi_cmdlist_put(ctrl, &cmdreq);		//SW4-HL-Display-EnhanceErrorHandling-00*_20170518
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	pr_debug("[HL]%s, %d: enable = %d <-- START\n", __func__, __LINE__, enable);
	...
	if (enable)
	{
		pr_debug("[HL]%s, %d, enable = 1\n", __func__, __LINE__);

		pr_debug("[HL]%s, %d, mdss_dsi_request_gpios() <-- START\n", __func__, __LINE__);
		rc = mdss_dsi_request_gpios(ctrl_pdata);
		pr_debug("[HL]%s, %d, mdss_dsi_request_gpios() <-- END\n", __func__, __LINE__);
		if (rc) {
		...
		{
			pr_debug("[HL]%s, %d, pinfo->cont_splash_enabled is FALSE\n", __func__, __LINE__);
		...
		{
			pr_debug("[HL]%s, %d, pinfo->cont_splash_enabled is TRUE, NOOOOOOOOOOOO PULL RESET PIN!\n", __func__, __LINE__);
		}
	...
	else
	{
		...
		pr_debug("[HL]%s, %d, gpio_set_value((ctrl_pdata->rst_gpio), 0)\n", __func__, __LINE__);
		gpio_free(ctrl_pdata->rst_gpio);
		pr_debug("[HL]%s, %d, gpio_free(ctrl_pdata->rst_gpio))\n", __func__, __LINE__);
		if (gpio_is_valid(ctrl_pdata->mode_gpio))
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (on_cmds->cmd_cnt)
	{
		if (0)	//if (strstr(saved_command_line, "androidboot.device=EAG") != NULL)
		{
			if (!(strnstr(saved_command_line, "androidboot.fihmode=2", strlen(saved_command_line))))
			{
				switch (ctrl->panel_data.panel_info.panel_id)
				{
					...
					case FIH_ST7703_INX_HD_PLUS_VIDEO_PANEL:
						{
							pr_debug("[HL]%s, %d: case FIH_ST7703_TRULY_HD_PLUS_VIDEO_PANEL or case FIH_ST7703_CTC_HD_PLUS_VIDEO_PANEL OR FIH_ST7703_INX_HD_PLUS_VIDEO_PANEL\n", __func__, __LINE__);
						}
						break;
					...
					case FIH_NT36525_HD_PLUS_INNO_VIDEO_PANEL:
						{
							pr_debug("[HL]%s, %d: case FIH_HX83102_HD_PLUS_VIDEO_PANEL or FIH_HX83102_HD_PLUS_INNO_VIDEO_PANEL or FIH_NT36525_HD_PLUS_INNO_VIDEO_PANEL\n", __func__, __LINE__);
						}
						break;
				}
                ...
                pr_debug("[HL]%s, %d: mdss_dsi_panel_cmds_send(ctrl, on_cmds, CMD_REQ_COMMIT)\n", __func__, __LINE__);
		...
		if (strstr(saved_command_line, "androidboot.device=EAG") != NULL)
		{
			...
			if (0)	//if (!(strnstr(saved_command_line, "androidboot.fihmode=2", strlen(saved_command_line))))
			{
				//if (ctrl->switch_cmdpage_cmds.cmd_cnt)
				//{
				//	len = mdss_dsi_panel_cmds_send(ctrl, &ctrl->switch_cmdpage_cmds, CMD_REQ_COMMIT);
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	if (ctrl->off_cmds.cmd_cnt)
	{
		...
		pr_debug("[HL]%s, %d: mdss_dsi_panel_cmds_send(ctrl, &ctrl->off_cmds, CMD_REQ_COMMIT) <-- START\n", __func__, __LINE__);
	...
	switch (ctrl->panel_data.panel_info.panel_id)
	{
		case FIH_HX8352_TM_240x400_VIDEO_PANEL:
			...
			mdelay(100);
			pr_err("[HL]%s, %d: [LCM ENTER SLEEP IN MODE]\n", __func__, __LINE__);
			break;
		default:
			break;
	}
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->cabc_off_cmds_beforeInit,
		"qcom,mdss-dsi-cabc-off-command-BeforeInit", "qcom,mdss-dsi-cabc-off-command-BeforeInit-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->cabc_ui_cmds_beforeInit,
		"qcom,mdss-dsi-cabc-ui-command-BeforeInit", "qcom,mdss-dsi-cabc-ui-command-BeforeInit-state");
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->cabc_still_cmds_beforeInit,
		"qcom,mdss-dsi-cabc-still-command-BeforeInit", "qcom,mdss-dsi-cabc-still-command-BeforeInit-state");
