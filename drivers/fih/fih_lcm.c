...
extern void fih_get_tp_data (char *color_data);        //FIHTDC - gatycclu - OC6 - Show panel tp data
...
static int fih_lcm_read_tp_data(struct seq_file *m, void *v)
{
       char tp_data[20]={0};

       fih_get_tp_data(tp_data);

       seq_printf(m, "%s", tp_data);

       return 0;
}

static int fih_lcm_tp_data_open(struct inode *inode, struct file *file)
{
       return single_open(file, fih_lcm_read_tp_data, NULL);
};

static struct file_operations tp_data_file_ops = {
       .owner   = THIS_MODULE,
       .open    = fih_lcm_tp_data_open,
       .read    = seq_read,
       .write   = NULL,
       .llseek  = seq_lseek,
       .release = single_release
	...
	size_t count, loff_t *ppos)
{
	...
	if(strstr(saved_command_line, "androidboot.fihmode=0")!=NULL)	//SW4-HL-Display-FixCABCCanNotWorkInAndroidO-00*_20171020
	{
...
struct {
	...
	{NULL}, },
	LCM0_tp_data[] = {  //FIHTDC - gatycclu - OC6 - Show panel tp data
	{"AllHWList/LCM0/tp_data", &tp_data_file_ops},
	#if 0
	{NULL}, },
	//SW4-HL-Display-PowerPinControlPinAndInitCodeAPI-00+{_20150519
	LCM0_vddio[] = {
...
static int __init fih_lcm_init(void)
{
	...
	//FIHTDC - gatycclu - OC6 - Show panel color data{
	if(fih_hwid_fetch(FIH_HWID_PRJ) == FIH_PRJ_OD6) {
		...
		ent = proc_create((LCM0_tp_data->name) + 15, 0, lcm0_dir, LCM0_tp_data->ops);
		if (ent == NULL)
		{
			pr_err("\n\nUnable to create /proc/%s", LCM0_tp_data->name);
		}
		pr_debug("\n\n*** [HL] %s, succeed to create proc/%s ***\n\n", __func__, LCM0_tp_data->name);
	}
	//FIHTDC - gatycclu - OC6 - Show panel color data}
...
static void __exit fih_lcm_exit(void)
{
	...
	if(fih_hwid_fetch(FIH_HWID_PRJ) == FIH_PRJ_OD6) {
		...
		remove_proc_entry(LCM0_tp_data->name, NULL);
	}
	//FIHTDC - gatycclu - OC6 - Show panel color data}
