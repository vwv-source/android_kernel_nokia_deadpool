...
int fih_nv_read(char *FIH_WLAN_NV_FILE , size_t size)
{
    ...
    if (IS_ERR(file_filp)) {
        wcnss_log(ERR, "%s : Fail to open %s", __FUNCTION__,PROJ_ID_PATH);
        return -1;
    }
    if (!file_filp->f_op || !file_filp->f_op->read) {
        filp_close(file_filp, NULL);
        wcnss_log(ERR, "%s : File (read) object is a null pointer!!!", __FUNCTION__);
        return -1;
    }
    ...
    //Search NV table
    wcnss_log(ERR, "/proc/devmodel: %s", proj_id);
    ...
    for(i=0;i< NV_TABLE_SIZE(g_fih_wlan_nv); i++) {
        if( strncmp( proj_id, g_fih_wlan_nv[i].project_name, strlen(proj_id)) == 0) {
            ...
            wcnss_log(ERR, "Matching proj_id: %s, file_path=%s", proj_id, file_path);

            //check file exist?
            snprintf(check_path, size,"%s/%s",FW_PATH,file_path);
            wcnss_log(ERR, "Real File Patch=%s",check_path);
            file_filp = filp_open(check_path, O_RDONLY, 0);
            if(IS_ERR(file_filp)){
                //file isn't exsit
                wcnss_log(ERR, "%s(%d) : Fail to open %s", __FUNCTION__, __LINE__,check_path);
                return -1;
            }else{
                wcnss_log(ERR, "File exist on %s",check_path);
                found = 1;
            }
        }
    }
    ...
    if(found ==0){
       // no found 
       wcnss_log(ERR, "No match proje_id(%s) found, use QC default path.",proj_id);
...
static void wcnss_nvbin_dnld(void)
{
    ...
    if( fih_nv_read(FIH_WLAN_NV_FILE, sizeof(FIH_WLAN_NV_FILE)) < 0){
        ...
        wcnss_log(ERR, "fih_nv_read ret= -1, use QC default path, FIH_WLAN_NV_FILE=%s",FIH_WLAN_NV_FILE);
    }else{
        wcnss_log(ERR, "fih_nv_read ret= 1, use FIH NV FILE:%s \n",FIH_WLAN_NV_FILE);
    }
...
#else //QC default
    wcnss_log(ERR, "%s default QC route, not fih route \n",__func__);
