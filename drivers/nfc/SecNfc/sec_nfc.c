	...
	struct sec_nfc_platform_data *pdata)
{
	...
	pdata->pw = of_get_named_gpio(np, "sec-nfc,pow-gpio", 0);
  ...
  //Alan Modify, B
	if (gpio_is_valid(pdata->clk_req)) {
		r = gpio_request(pdata->clk_req, "sec_clkreq_gpio");
		if (r) {
			pr_info("%s: unable to request nfc clkreq gpio [%d]\n",
				__func__, pdata->clk_req);
			return -1;
		}
		r = gpio_direction_input(pdata->clk_req);
		if (r) {
			pr_info("%s: cannot set direction for nfc clkreq gpio [%d]\n",
			__func__, pdata->clk_req);
		}
	} else {
		pr_info("%s: clkreq gpio not provided\n", __func__);
	}
  //Alan Modify, E
	pr_info("Enabled clock\n");
	clk_prepare_enable(pdata->clk);

#endif

	pr_info("%s: irq : %d, ven : %d, firm : %d, vdd : %d\n",
			__func__, pdata->irq, pdata->ven, pdata->firm, pdata->pw);
	return 0;
}
...
static int __sec_nfc_probe(struct device *dev)
{
	...
	if (!info) {
		pr_info("failed to allocate memory for sec_nfc_info\n");
		ret = -ENOMEM;
		goto err_info_alloc;
	}
	info->dev = dev;
	...
	if (pdata->pw)
	{
		ret = gpio_request(pdata->pw, "nfc_pw");
		if (ret) {
		pr_info("failed to get gpio power");
		goto err_gpio_pw;
		}
	}
        gpio_direction_output(pdata->pw, SEC_NFC_VDD_ON);
	ret = gpio_request(pdata->ven, "nfc_ven");
	if (ret) {
		pr_info("failed to get gpio ven\n");
		goto err_gpio_ven;
	}
	...
	if (pdata->firm)
	{
		...
		if (ret) {
			pr_info("failed to get gpio firm\n");
			goto err_gpio_firm;
		}
	...
	pr_info("%s success info: %p pdata %p.\n",__func__, info, pdata);
	return 0;
...
static int __sec_nfc_remove(struct device *dev)
{
	...
	if (pdata->pw) gpio_free(pdata->pw);
	free_irq(client->irq, info);
	gpio_free(pdata->irq);
