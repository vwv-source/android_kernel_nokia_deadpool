		...
		else // dual cam re-calibration case
		{
			...
			for (i = 0; i < eeprom_map->memory_map_size; i++) {
				...
				case MSM_CAM_WRITE: {
					...
					pr_info("%s: addr[0x%X] = 0x%X\n", __func__, eeprom_map->mem_settings[i].reg_addr, data[num_byte]);
					num_byte++;
	...
	void *argp)
{
	...
	case CFG_EEPROM_GET_CAL_DATA:
		...
		cdata->cfg.get_data.num_bytes =
			e_ctrl->cal_data.num_data;
		pr_err("%s:%d num_bytes = %d", __func__, __LINE__, cdata->cfg.get_data.num_bytes);
		break;
...
static int msm_eeprom_platform_probe(struct platform_device *pdev)
{
	...
	rc = of_property_read_string(of_node, "qcom,eeprom-name",
		&eb_info->eeprom_name);
	CDBG("%s,%d qcom,eeprom-name %s, rc %d\n", __func__, __LINE__,
		eb_info->eeprom_name, rc);
	if (rc < 0) {
		pr_err("%s failed %d\n", __func__, __LINE__);
		e_ctrl->userspace_probe = 1;
...
#if 0
		if(!strncmp("s5k3l6_radiohead_main", eb_info->eeprom_name, sizeof("s5k3l6_radiohead_main"))){
			pr_err("read s5k3l6_radiohead_main data\n");
			for (j = 0; j < e_ctrl->cal_data.num_data; j++)
				pr_err("eeprom_data[0x%X] = %d (0x%X)\n", j, e_ctrl->cal_data.mapdata[j],
					e_ctrl->cal_data.mapdata[j]);
		}
#endif
		e_ctrl->is_supported |= msm_eeprom_match_crc(&e_ctrl->cal_data);
