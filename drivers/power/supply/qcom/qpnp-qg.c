...
#define QPNPQG_BATTERY_VOLTAGE_LOW do {printk("BBox;%s: Voltage low\n", __func__); printk("BBox::UEC;49::3\n");} while (0)
...
static bool is_batt_empty(struct qpnp_qg *chip)
{
	int rc;
	int vbatt_uv;
	u8 status = 0;
	...
	if (!(status & BATTERY_EMPTY_INT_RT_STS_BIT))
		return false;

	pr_err("BATTERY_EMPTY_INT_RT_STS_BIT\n");
	...
	if(vbatt_uv < chip->dt.vbatt_empty_cold_mv * 1000)
		QPNPQG_BATTERY_VOLTAGE_LOW;

	return ((vbatt_uv < chip->dt.vbatt_empty_cold_mv * 1000) ? true : false);
}
...
// FIHTDC, IdaChiang, add for OD6 100K NTC {{
	if(of_find_property(profile_node, "qcom,fih-batt-therm-pu", NULL))
	{
		rc = of_property_read_u32(profile_node,
				"qcom,fih-batt-therm-pu",
				&chip->bp.fih_batt_therm_pu);
		if (rc < 0)
			chip->bp.fih_batt_therm_pu = 0;
	}
	else
		chip->bp.fih_batt_therm_pu = 0;
// FIHTDC, IdaChiang, add for OD6 100K NTC }}
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
	...
	if (chip->bp.fih_profile_version > 0)
	{
		rc = qg_sdam_multibyte_read(
			QG_SDAM_PROFILE_VERSION_OFFSET,
			&sdam_profile_version, 1);
		pr_info("sdam_profile_version=%x, chip->bp.fih_profile_version=%x \n", sdam_profile_version, chip->bp.fih_profile_version);
		if (rc < 0)
			pr_err("Failed to read SDAM rc=%d\n", rc);
		else if (chip->bp.fih_profile_version
				!= sdam_profile_version) {
			pr_err("do battery removal for new battery profile \n");	
			rc = qg_handle_battery_removal(chip);
			if (rc < 0)
				pr_err("Failed to clear SDAM rc=%d\n",
								 rc);
...
// FIHTDC, IdaChiang, add for BBX log {{
	rc = qg_get_nominal_capacity(&nom_cap_uah, 250, true);
...
//	printk("BBox::UPD;50::%s::%lld\n", qg_get_cycle_counts(chip), (temp/1000));

	if(chip->battery_missing == true)
	{
		printk("BBox;%s: Battery missing\n", __func__); 
		printk("BBox::UEC;11::2\n");
	}
	
	chip->fih_current_lvl =0;
// FIHTDC, IdaChiang, add for BBX log }}
