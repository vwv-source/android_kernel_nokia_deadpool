...
#include "../../../input/touchscreen/nt36xxx/nt36xxx.h"	//SW4-HL-Touch-UsbPlugInOut-00+_20181029
			...
			int count)
{
	...
	rc = regmap_bulk_write(chg->regmap, addr, val, count);
	if(rc<0)
		CHARGER_WRITE_ERROR;
	return rc;
}
...
static const struct apsd_result *smblib_get_apsd_result(struct smb_charger *chg)
{
	...
	if(chg->od6_prj_only)
	{
		if (gpio_is_valid(chg->chg_terminal_pin)) {
			gpio_direction_output((chg->chg_terminal_pin), 1);
			gpio_direction_input((chg->chg_terminal_pin));
			chg_termina_det = gpio_get_value((chg->chg_terminal_pin));
		}
		if(chg_termina_det == 0)
		{
			pr_err("Charging terminal enable, set source to DCP\n");
	...
	{
		if (gpio_is_valid(chg->chg_terminal_pin)) {
			gpio_direction_output((chg->chg_terminal_pin), 0);
			gpio_direction_input((chg->chg_terminal_pin));
			chg_termina_det = gpio_get_value((chg->chg_terminal_pin));
		
			if(chg_termina_det == 1)
			{
				pr_err("Charging terminal enable, set source to DCP\n");
				result = &smblib_apsd_results[DCP];
			}
...
void fih_od6_set_otg_enable(struct smb_charger *chg, int enable)
{
	if(chg->od6_prj_only || chg->lms_prj_only)
	{
		if (gpio_is_valid(chg->od6_otg_enable_pin)) {
			if(enable == 0)
				gpio_set_value((chg->od6_otg_enable_pin), 0);
			else if(enable == 1)
				gpio_set_value((chg->od6_otg_enable_pin), 1);
		}
	}
				...
				union power_supply_propval *val)
{
	...
	if(chg->od6_prj_only && stat == DISABLE_CHARGE)
		val->intval = POWER_SUPPLY_STATUS_DISCHARGING;
	...
	if(chg->od6_prj_only)
	{
		...
		stat &= CHARGER_ERROR_STATUS_BAT_OV_BIT | CHARGER_ERROR_STATUS_SFT_EXPIRE;
			       ...
			       union power_supply_propval *val)
{
	...
	/* If USB path is present and flash is active always report 1 */
	if ((stat & USE_USBIN_BIT) && is_flashing(chg))
		val->intval = 1;
	else
					...
					int usb_current)
{
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
		if (usb_current == -ETIMEDOUT) {
			...
			if(chg->fih_dcp_2a_enable == 1)
				rc = vote(chg->usb_icl_votable,
					SW_ICL_MAX_VOTER, true, DCP_2A_CURRENT_UA);
			else
				rc = vote(chg->usb_icl_votable,
...
void smblib_usb_pm_lock(struct smb_charger *chg)
{
	int rc;
	u8 stat;
	bool vbus_rising;

	if(chg->od6_prj_only != true)
		return;
	...
	if(vbus_rising)
	{
		if(!wake_lock_active(&chg->od6_chg_wake_lock))
		{
			pr_err("%s od6_chg_wake_lock on\n", __func__);
			wake_lock(&chg->od6_chg_wake_lock);
		}
	}
	else
	{
		if(wake_lock_active(&chg->od6_chg_wake_lock))
		{
			pr_err("%s od6_chg_wake_lock off\n", __func__);
			wake_unlock(&chg->od6_chg_wake_lock);
		}
	}
}
...
uint8_t bUsbState = 0;	//SW4-HL-Touch-UsbPlugInOut-00+_20181029
irqreturn_t typec_state_change_irq_handler(int irq, void *data)
{
	...
	//SW4-HL-Touch-UsbPlugInOut-00+{_20181029
	if ((strstr(saved_command_line, "androidboot.device=RHD") != NULL) && (strstr(saved_command_line, "fih,mdss_dsi_nt36525_inno_hd_plus_video") != NULL))
	{
		pr_err("[HL]%s, %d: bTouchIsAwake = %d\n", __func__, __LINE__, bTouchIsAwake);
		if (bTouchIsAwake)
		{
			pr_err("[HL]%s, %d: chg->typec_mode = %d\n", __func__, __LINE__, chg->typec_mode);
			pr_err("[HL]%s, %d: smblib_typec_mode_name[chg->typec_mode] = (%s)\n", __func__, __LINE__, smblib_typec_mode_name[chg->typec_mode]);
			switch (chg->typec_mode)
			{
				case 0:	//USB state: NONE
					pr_err("[HL]%s, %d: chg->typec_mode = 0", __func__, __LINE__);
					tpd_usb_plugin(USB_PLUGOUT);
					bUsbState = USB_PLUGOUT;
					break;
				case 6:	//USB state: SOURCE-DEFAULT
					pr_err("[HL]%s, %d: chg->typec_mode = 6", __func__, __LINE__);
					tpd_usb_plugin(USB_PLUGIN);
					bUsbState = USB_PLUGIN;
					break;
				default:
					pr_err("[HL]%s, %d: chg->typec_mode = default = %d", __func__, __LINE__, chg->typec_mode);
					break;
			}
		}
	}
...
EXPORT_SYMBOL(bUsbState);	//SW4-HL-Touch-UsbPlugInOut-00+{_20181029
...
void dump_register(struct smb_charger *chip, int index)
{
	int rc;
	int i=0;
	u8 reg0 =0, reg1 =0, reg2 =0, reg3 =0, reg4 =0, reg5 =0, reg6 =0, reg7 =0, reg8 =0, reg9 =0, rega =0, regb =0, regc =0, regd =0, rege =0, regf =0;
...
{
	int rc;
	u8 reg1006 =0, reg1007 =0, reg1008 =0, reg1009 =0, reg100a =0, reg100b =0, reg100c =0, reg100d =0, reg100e =0, reg1061 =0, reg1070 =0;
	u8 reg1306 =0, reg1307 =0, reg1308 =0, reg1309 =0, reg130a =0, reg130b =0, reg1340 =0;
	u8 reg1506 =0, reg1508 =0, reg1509 =0, reg150a =0, reg150b =0, reg150c =0, reg150d =0, reg150e =0, reg150f =0;
	u8 reg1a08 =0, reg1a09 =0;

	rc = smblib_read(chip, 0x1006, &reg1006);
	rc = smblib_read(chip, 0x1007, &reg1007);
	rc = smblib_read(chip, 0x1008, &reg1008);
	...
	rc = smblib_read(chip, 0x130a, &reg130a);
	rc = smblib_read(chip, 0x130b, &reg130b);
	rc = smblib_read(chip, 0x1340, &reg1340);

	rc = smblib_read(chip, 0x1506, &reg1506);
	rc = smblib_read(chip, 0x1508, &reg1508);
	rc = smblib_read(chip, 0x1509, &reg1509);
	rc = smblib_read(chip, 0x150a, &reg150a);
	rc = smblib_read(chip, 0x150b, &reg150b);
	rc = smblib_read(chip, 0x150c, &reg150c);
	rc = smblib_read(chip, 0x150d, &reg150d);
	rc = smblib_read(chip, 0x150e, &reg150e);
	rc = smblib_read(chip, 0x150f, &reg150f);

	rc = smblib_read(chip, 0x1a08, &reg1a08);
	rc = smblib_read(chip, 0x1a09, &reg1a09);

	pr_err("BATTLog C:S%02xS%02xS%02xS%02xS%02xS%02xS%02xS%02xS%02xC%02xC%02x, U:S%02xS%02xS%02xS%02xS%02xS%02xC%02x, C:S%02xS%02xS%02xS%02xS%02xS%02xS%02xS%02xS%02x, B:S%02xS%02x\n",
		reg1006, reg1007, reg1008, reg1009, reg100a, reg100b, reg100c, reg100d, reg100e, reg1061, reg1070,
		reg1306, reg1307, reg1308, reg1309, reg130a, reg130b, reg1340,
		reg1506, reg1508, reg1509, reg150a, reg150b, reg150c, reg150d, reg150e, reg150f,
		reg1a08, reg1a09);
...
//	pmi_temp = val.intval;
	...
	if(pmiFail == false) {// if the pmic is unstable, we will not show the log to get rid of flooding info.
		pr_err("BATTLog ISD=%s,UOL=%s,UPR=%s,CM=%s,UIS=%d,CAP=%d,VB=%d,IB=%d,FCC(%s)=%d,ICL(%s)(%s)=%d,BS=%d,BT=%d,CV=%d,CI=%d,MTS=%d,TL=%d\n",
			input_suspend == true ? "T" : "F",
			usb_online == true ? "T" : "F",
			usb_present == true ? "T" : "F",
			typec_mode,
...
//		dump_register(chg, 0x1400);
	if (*chg->fih_dump_mask & BIT(5))
		dump_register(chg, 0x1500);
...
//		dump_register(chg, 0x1700);
	...
	msecs_to_jiffies(FIH_WORK_TIME));
}
/* FIH Function End */

// FIHTDC, IdaChiang, add for recharging {{
static void fih_set_recharging_setting_work(struct work_struct *work)
{
	int rc =0;
	union power_supply_propval val = {0, };
	bool input_suspend = false;
	...
	int charge_done = 0;
	struct smb_charger *chg = container_of(work, struct smb_charger,
							set_recharging_setting_work.work);
	...
	if(input_suspend == true)
		return;

	smblib_get_prop_batt_charge_done(chg, &val);
	charge_done = (val.intval == 1) ? true : false;

	if(charge_done == true)
		return;

	pr_err("set recharge for 98 capacity full issue\n");
	val.intval = 0;
...
static void smblib_dr_role_check_work(struct work_struct *work)
{
	...
	switch (chg->dr_mode) {
	case DUAL_ROLE_PROP_MODE_UFP:
		if (chg->sink_src_mode != SINK_MODE) {
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->update_batt_info_work, fih_update_batt_info_work);// FIHTDC, IdaChiang, add dump log
	INIT_DELAYED_WORK(&chg->role_reversal_check, smblib_dr_role_check_work);
	INIT_DELAYED_WORK(&chg->set_recharging_setting_work, fih_set_recharging_setting_work);// FIHTDC, IdaChiang, add dump log

	if(chg->od6_prj_only)
		wake_lock_init(&chg->od6_chg_wake_lock, WAKE_LOCK_SUSPEND, "od6_chg_wake_lock");
	...
	{
		if (gpio_is_valid(chg->od6_otg_enable_pin)) {
			rc = gpio_request(chg->od6_otg_enable_pin,	"od6_otg_enable");
			if (rc)
				pr_err("request otg_enable, rc=%d\n", rc);
			else
			{
				gpio_direction_output((chg->od6_otg_enable_pin), 0);
			}
		}

		if (gpio_is_valid(chg->chg_terminal_pin)) {
			rc = gpio_request(chg->chg_terminal_pin,	"chg_terminal_detect");
			if (rc)
				pr_err("request chg_terminal_detect, rc=%d\n", rc);
			else
			{
				gpio_direction_output((chg->chg_terminal_pin), 1);
			}
...
int smblib_post_init(struct smb_charger *chg)
{
	...
	schedule_delayed_work(&chg->update_batt_info_work, msecs_to_jiffies(FIH_WORK_TIME));
	schedule_delayed_work(&chg->set_recharging_setting_work, msecs_to_jiffies(FIH_WORK_TIME));
