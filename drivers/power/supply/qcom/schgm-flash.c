...
bool is_flashing(struct smb_charger *chg)
{
	int rc;
	u8 reg;
	
	rc = smblib_read(chg, SCHGM_FLASH_STATUS_1_REG, &reg);
	if (rc < 0) {
		pr_err("Couldn't read flash status_1 rc=%d\n", rc);
		return 0;
	}

	pr_debug("Flash status_1=[%x]\n", reg);

	return !!(reg && FLASH_ACTIVE_BIT);
}
