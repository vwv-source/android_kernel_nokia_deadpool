...
#define FG_READ_ERROR	do {printk("BBox;%s: qg read failed\n", __func__); printk("BBox::UEC;12::2\n");} while (0)
#define FG_WRITE_ERROR	do {printk("BBox;%s: qg write failed\n", __func__); printk("BBox::UEC;12::3\n");} while (0)
...
int qg_get_battery_temp(struct qpnp_qg *chip, int *temp)
{
	...
	result.fih_cur_lvl = chip->fih_current_lvl;
	if(chip->bp.fih_batt_therm_pu == 2)
		rc = qpnp_vadc_read(chip->vadc_dev, VADC_BAT_THERM_PU2, &result); // FIHTDC, IdaChiang, add for ntc 100k, use 100k pull-up
	else
		rc = qpnp_vadc_read(chip->vadc_dev, VADC_BAT_THERM_PU1, &result); // FIHTDC, IdaChiang, add for ntc 10k, use 30k pull-up
	if (rc) {
