...
// path: /sys/module/qpnp_smb5/parameters/fih_dump_mask
...
ssize_t type_c_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(rc < 0){
		pr_err("%s:Couldn't read TYPE_C_SNK_STATUS_REG rc=%d\n", __func__, rc);
		stat = 0;
	}
	pr_info("%s: stat=0x%x\n", __func__, stat);
	if(stat == 0x08)
...
int  jeita_full_capacity_get_status(struct smb_charger *chg)
{
	...
	if(bat_temp >= JEITA_WARM_TEMP && usb_online == true) {
...
static int smb5_parse_dt(struct smb5 *chip)
{
	...
	// FIHTDC, IdaChiang, add for warm temp recharging {{
	chg->auto_recharge_vbat_mv = chip->dt.auto_recharge_vbat_mv;
	chg->high_temp_rechg_vbat_mv = -EINVAL;
	rc = of_property_read_u32(node, "fih,auto-rechg-high-vbat-mv",
				&chg->high_temp_rechg_vbat_mv);
	if (!rc && (chg->high_temp_rechg_vbat_mv < 0)) {
		pr_err("fih,auto-rechg-high-vbat-mv is incorrect\n");
	}
	pr_err("auto_recharge_mv=%d, %d \n", chg->auto_recharge_vbat_mv, chg->high_temp_rechg_vbat_mv);
	// FIHTDC, IdaChiang, add for warm temp recharging }}

	chg->dcp_icl_ua = chip->dt.usb_icl_ua;

	// FIHTDC, IdaChiang, add for 5V2A DCP
	chg->fih_dcp_2a_enable = 0;
	chg->fih_dcp_2a_enable = of_property_read_bool(node,
					"fih,dcp-2a-function");
...
//FIHTDC, IdaChiang, add for usb temperature {{
	if(chg->usb_thermal_function == true)
	{
		if (!chg->vadc_usb_dev) {
			if (of_find_property(chg->dev->of_node, "qcom,usb-vadc",
						NULL)) {
				chg->vadc_usb_dev = qpnp_get_vadc(chg->dev, "usb");
				if (IS_ERR(chg->vadc_usb_dev)) {
					rc = PTR_ERR(chg->vadc_usb_dev);
					if (rc != -EPROBE_DEFER)
						pr_err("Failed to find VADC node, rc=%d\n",
								rc);
					else
						chg->vadc_usb_dev = NULL;
		...
		if (IS_ERR(chg->vadc_usb_dev))
			return PTR_ERR(chg->vadc_usb_dev);
	}
//FIHTDC, IdaChiang, add for usb temperature }}
	...
	case USB_TEMP:
		if(chg->usb_thermal_function == true)
		{
			rc = qpnp_vadc_read(chg->vadc_usb_dev, VADC_AMUX_THM5, &result);
			if (rc < 0) {
				pr_err("Failed to read USB_TEMP over vadc, rc=%d\n", rc);
				goto done;
			}
			val->intval = ((int)result.physical)/100; // 0.1 degC
		}
		else
			val->intval = 250;
		break;
...
// FIHTDC, IdaChiang, add for USB temperature {{
// disable cc-logic, usb_lvl 0: enable cc-logic, 1: disable cc-logic
static int smb5_set_usb_thermal_mitigation(struct smb_charger *chg, int usb_lvl)
{
	int rc =0;

	if(usb_lvl == 1)
	{
		chg->usb_thermal_mitigation = 1;
		rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
					TYPEC_DISABLE_CMD_BIT, 0x1);
		rc = smblib_write(chg, 0x155c, 0x0f); // CC pin to open
	}
	else
	{
		chg->usb_thermal_mitigation = 0;
		...
		rc = smblib_write(chg, 0x155c, 0x00); // CC pin to hw control
	}
	return rc;
...
static int smb5_set_recharging_mv(struct smb_charger *chg, int val_u)
{
	u32 temp;
	int rc =0;
	...
	if (chg->auto_recharge_vbat_mv != -EINVAL && chg->high_temp_rechg_vbat_mv != -EINVAL ) {

		if(val_u == chg->batt_profile_fv_uv)
			temp = VBAT_TO_VRAW_ADC(chg->auto_recharge_vbat_mv);
		else
			temp = VBAT_TO_VRAW_ADC(chg->high_temp_rechg_vbat_mv);

		if(chg->pre_recharge_val == temp)
			return rc;

		chg->pre_recharge_val = temp;
		pr_err("auto_recharge_vbat_mv  = %d \n", chg->pre_recharge_val );
...
// FIHTDC, IdaChiang, add for usb temperature {{
	case POWER_SUPPLY_PROP_TEMP:
		...
		{
			if (chg->smb_version == PMI632_SUBTYPE)
				rc = smb5_get_adc_data(chg, USB_TEMP, val);
			else
				val->intval = 250;
		}
		else
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT_MAX:
		val->intval = 2; // 0: enable cc-logic, 1: disable cc-logic
		break;
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		val->intval = chg->usb_thermal_mitigation;
		break;
// FIHTDC, IdaChiang, add for usb temperature }}
	default:
...
// FIHTDC, IdaChiang, add for usb temperature {{
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		 // 0: enable cc-logic, 1: disable cc-logic
		 rc = smb5_set_usb_thermal_mitigation(chg, val->intval);
		break;
// FIHTDC, IdaChiang, add for usb temperature }}
	default:
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		smb5_set_recharging_mv(chg, val->intval); // FIHTDC, IdaChiang, add for warm temp recharging
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if(chip->dt.hvdcp_disable == true)
	{
		pr_err("disable HVDCP\n");
		rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG,
						HVDCP_EN_BIT, 0);
	}
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	if (rc < 0) {
		pr_err("smblib_post_init failed rc=%d\n", rc);
		return rc;
	}
	...
	rc = device_create_file(&pdev->dev, &dev_attr_jeita_full_capacity); // FIHTDC, IdaChiang, add for battery protect apk
	return rc;
