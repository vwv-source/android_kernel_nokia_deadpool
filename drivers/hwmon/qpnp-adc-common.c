...
// FIHTDC, IdaChiang, add for battery NTC
static const struct qpnp_vadc_map_pt adcmap_batt_therm_fih_B3435_10K[] = {
	{1673,	-400},
	{1649,	-380},
	{1623,	-360},
	{1596,	-340},
...
// FIHTDC, IdaChiang, add for battery NTC
static const struct qpnp_vadc_map_pt adcmap_batt_therm_fih_B4250_100K[] = {
	{1840,	-400},
	{1835,	-380},
	{1828,	-360},
	{1821,	-340},
...
// FIHTDC, IdaChiang, add for battery NTC
int32_t qpnp_adc_batt_therm_fih_b3435_10k(struct qpnp_vadc_chip *chip,
		int32_t adc_code,
		const struct qpnp_adc_properties *adc_properties,
		const struct qpnp_vadc_chan_properties *chan_properties,
		struct qpnp_vadc_result *adc_chan_result)
{
	...
	if (adc_properties->adc_hc) {
		...
		qpnp_adc_map_voltage_temp(adcmap_batt_therm_fih_B3435_10K,
			ARRAY_SIZE(adcmap_batt_therm_fih_B3435_10K),
			batt_thm_voltage, &adc_chan_result->physical);
	}
	//pr_err("batt therm adc_code =%x, voltage = %lld, temp = %lld c, \n", adc_code, batt_thm_voltage, adc_chan_result->physical);
	return 0;
}
EXPORT_SYMBOL(qpnp_adc_batt_therm_fih_b3435_10k);

// FIHTDC, IdaChiang, add for EAG battery NTC
int32_t qpnp_adc_batt_therm_fih_b3435_10k_eag(struct qpnp_vadc_chip *chip,
		int32_t adc_code,
		const struct qpnp_adc_properties *adc_properties,
		const struct qpnp_vadc_chan_properties *chan_properties,
		struct qpnp_vadc_result *adc_chan_result)
...
// FIHTDC, IdaChiang, add for therm pin voltage issue {{
	if(adc_chan_result->fih_cur_lvl>= 0)
		vbat_offset = 0;
	else
		vbat_offset = (((adc_chan_result->fih_cur_lvl/1000) * (-16))/1000); // V=I*R, R = 0.016 Ohm
// FIHTDC, IdaChiang, add for therm pin voltage issue }}
	...
	if (adc_properties->adc_hc) {
		...
		// FIHTDC, IdaChiang, add for therm pin voltage issue {{
		if(vbat_offset >0)
			batt_thm_voltage = batt_thm_voltage -vbat_offset;
		// FIHTDC, IdaChiang, add for therm pin voltage issue }}
	...
	//pr_err("batt therm adc_code =%x, voltage = %lld, temp = %lld c, vbat_offset=%d \n", adc_code, batt_thm_voltage, adc_chan_result->physical, vbat_offset);
	return 0;
}
EXPORT_SYMBOL(qpnp_adc_batt_therm_fih_b3435_10k_eag);

// FIHTDC, IdaChiang, add for OD6 battery NTC
int32_t qpnp_adc_batt_therm_fih_b4250_100k(struct qpnp_vadc_chip *chip,
		int32_t adc_code,
		const struct qpnp_adc_properties *adc_properties,
		const struct qpnp_vadc_chan_properties *chan_properties,
		struct qpnp_vadc_result *adc_chan_result)
{
	...
	if (adc_properties->adc_hc) {
		...
		qpnp_adc_map_voltage_temp(adcmap_batt_therm_fih_B4250_100K,
			ARRAY_SIZE(adcmap_batt_therm_fih_B4250_100K),
			batt_thm_voltage, &adc_chan_result->physical);
	}
	//pr_err("batt therm adc_code =%x, voltage = %lld, temp = %lld c, \n", adc_code, batt_thm_voltage, adc_chan_result->physical);
	return 0;
}
EXPORT_SYMBOL(qpnp_adc_batt_therm_fih_b4250_100k);
