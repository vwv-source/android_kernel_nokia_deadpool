...
void fih_dump_pon_data(struct qpnp_pon *pon)
{
	int rc = 0;
	int usid = 0;
	uint reg8c0;// pon reason1
	uint reg8c2;// warm reset reason1
	uint reg8c4;// on reason
	uint reg8c5;// poff reason1
	uint reg8c7;// off reason
	uint reg8c8;// fault reason1
	uint reg8c9;// fault reason2
	uint reg8ca;// s3 reset reason
	uint reg8cb;// soft reset reason

	rc = regmap_read(pon->regmap, 0x08c0, &reg8c0);
	rc = regmap_read(pon->regmap, 0x08c2, &reg8c2);
	rc = regmap_read(pon->regmap, 0x08c4, &reg8c4);
	rc = regmap_read(pon->regmap, 0x08c5, &reg8c5);
	rc = regmap_read(pon->regmap, 0x08c7, &reg8c7);
	rc = regmap_read(pon->regmap, 0x08c8, &reg8c8);
	rc = regmap_read(pon->regmap, 0x08c9, &reg8c9);
	rc = regmap_read(pon->regmap, 0x08ca, &reg8ca);
	rc = regmap_read(pon->regmap, 0x08cb, &reg8cb);

	usid = to_spmi_device(pon->pdev->dev.parent)->usid;
	pr_err("%s, SID%d PON1(0): 0x%02x, Wreset1(2): 0x%02x, ON(4): 0x%02x, POF1(5): 0x%02x, OFF(7): 0x%02x, Fault1(8): 0x%02x, Fault2(9): 0x%02x, S3reset(a): 0x%02x, Softreset(b): 0x%02x\n",
			usid==0 ? "PM439" : "PMI632", usid,
			reg8c0, reg8c2, reg8c4, reg8c5, reg8c7, reg8c8, reg8c9, reg8ca, reg8cb);
