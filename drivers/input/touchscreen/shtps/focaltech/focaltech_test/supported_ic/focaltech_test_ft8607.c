...
#define FT8607_LEFT_KEY_REG             0X1E
#define FT8607_RIGHT_KEY_REG            0X1F
...
unsigned char FT8607_TestItem_LCDNoiseTest(bool* bTestResult);             //add by JanChen 20170913
unsigned char FT8607_CheckItem_ProjectCodeTest(bool* bTestResult);      //add by JanChen 20170913

/************************************************************************
* Name: FT8607_StartTest
...
boolean FT8607_StartTest()
{
    ...
    for (iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++)
    {
        ...
        {
	     for(i = 0 ; i < 3 ; i++){
	     	     printk("FT8607_TestItem_CbTest Count = %d \n",i);
	...
	///////////////////////////////////////////////////////FT8607_LCDNoise_TEST add by JanChen 20170913
        if (Code_FT8607_LCD_NOISE_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode)
        {
          #if TEST_ITEM
	     SysDelay(500);  //delay 500msec by Janchen 20171114
	     
	     ReCode_ChipClb = ChipClb( &bClbResult );
            if (ERROR_CODE_OK != ReCode_ChipClb)
            {
                  btmpresult = false;
                  FTS_TEST_ERROR("\r\n//========= auto clb Failed when LCD_NOISE_TEST!");
            }
          #endif
              for(i = 0 ; i < 5 ; i++){
                   printk("FT8607_TestItem_LCDNoiseTest Count = %d \n",i);
        ...
        ///////////////////////////////////////////////////////FT8607_PROJECT_CODE_TEST add by JanChen 20170913
        if (Code_FT8607_PROJECT_CODE_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode)
        {
            ReCode = FT8607_CheckItem_ProjectCodeTest(&bTempResult);
            if (ERROR_CODE_OK != ReCode || (!bTempResult))
            {
                bTestResult = false;
                g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
...
/************************************************************************
* Name: FT8607_TestItem_RawDataTest
...
//    bIncludeKey = g_stCfg_FT8607_BasicThreshold.bRawDataTest_VKey_Check;
        ...
        if (NULL == pfile)
            pfile = filp_open("/data/vendor/misc/touch/touch_data.txt", O_TRUNC|O_CREAT|O_RDWR, 0666);
        if (IS_ERR(pfile)){
...
unsigned char FT8607_TestItem_LCDNoiseTest(bool* bTestResult)
{
    ...
    FrameNum = g_stCfg_FT8607_BasicThreshold.LCDNoiseTest_FrameNum/4;
    ...
    if (0 == g_stCfg_FT8607_BasicThreshold.LCDNoiseTest_NoiseMode)
    {
        for (  iRow = 0; iRow <= g_stSCapConfEx.ChannelXNum; ++iRow )
        {
            for ( iCol = 0; iCol < g_stSCapConfEx.ChannelYNum; ++iCol )
    ...
    if (1 == g_stCfg_FT8607_BasicThreshold.LCDNoiseTest_NoiseMode)
    {
        for ( iRow = 0; iRow <= g_stSCapConfEx.ChannelXNum; ++iRow )
        {
            for (  iCol = 0; iCol < g_stSCapConfEx.ChannelYNum; ++iCol )
...
// VA
    iValueMin = 0;
    iValueMax = g_stCfg_FT8607_BasicThreshold.LCDNoiseTest_Coefficient*chNoiseValueVa* 32 / 100;    //
...
// Key
    iValueMin = 0;
    iValueMax = g_stCfg_FT8607_BasicThreshold.LCDNoiseTest_Coefficient_Key*chNoiseValueKey* 32 / 100;
...
/************************************************************************
* Name: FT8607_CheckItem_ProjectCodeTest
* Brief:   Factory Id Testt.
* Input: none
* Output: none
* Return: Comm Code. Code = 0x00 is OK, else fail.
***********************************************************************/
unsigned char FT8607_CheckItem_ProjectCodeTest(bool* bTestResult)
{
    unsigned char ReCode = ERROR_CODE_OK;
    bool btmpresult = true;
...
//  FTS_TEST_DBG("\r\nProject Code from Setting: %s "  g_stCfg_FT8607_BasicThreshold.Project_Code);
    if (strncmp(buffer,g_stCfg_FT8607_BasicThreshold.Project_Code,32)==0)
    {
        btmpresult = true;
    }
    else
...
static unsigned char GetChannelNum(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        ReCode = ReadReg( FT8607_LEFT_KEY_REG, &regData );
        if (ReCode == ERROR_CODE_OK)
        {
            if (((regData >> 0) & 0x01))
            {
        ...
        ReCode = ReadReg( FT8607_RIGHT_KEY_REG, &regData );
        if (ReCode == ERROR_CODE_OK)
        {
            if (((regData >> 0) & 0x01))
            {
...
unsigned char FT8607_GetTestResult(void)
{
    ...
    for (iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++)
    {
        ...
        ///////////////////////////////////////////////////////Code_FT8607_WEAK_SHORT_CIRCUIT_TEST
