...
/* -------------------------------------------------------------------------- */
static int shtps_fwctl_focaltech_get_chip_id(struct shtps_fwctl_info *fc_p, u16 *chipid_p)
{
	int rc=0;
	u8 buf[2] = { 0x00, 0x00 };
	...
	rc = M_READ_FUNC(fc_p, FTS_REG_CHIP_ID, &buf[0], 1);
	if(rc == 0){
		rc = M_READ_FUNC(fc_p, FTS_REG_CHIP_ID2, &buf[1], 1);
		if(rc == 0){
			*chipid_p = (buf[0] << 8) + buf[1];
		}
		else{
			*chipid_p = 0;
		}
...
struct shtps_fwctl_functbl		shtps_fwctl_focaltech_function_table = {
		...
		.get_chip_id_f							= shtps_fwctl_focaltech_get_chip_id,
};
/* -----------------------------------------------------------------------------------
*/
