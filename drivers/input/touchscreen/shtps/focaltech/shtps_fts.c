...
static void shtps_read_touchevent_insleep(struct shtps_fts *ts, int state)
{
  ...
  info.fingers[0].state = SHTPS_TOUCH_STATE_NO_TOUCH; //Must set initial value
...
static int shtps_wait_tp_to_valid(struct shtps_fts *ts)
{
	...
	do
	{
		ret = shtps_fwctl_get_chip_id(ts, &reg_value);
		chipID[0] = (reg_value >>8)&0x00FF;
		chipID[1] = (reg_value&0x00FF);

		if ((ret < 0) || (chipID[0] != FTS_CHIP_ID))
		{
			SHTPS_LOG_ERR_PRINT("TP Not Ready, ReadData = 0x%02x%02x\n", chipID[0], chipID[1]);
		}
		else if (chipID[0] == FTS_CHIP_ID)
		{
			SHTPS_LOG_ERR_PRINT("TP Ready, Device ID = 0x%02x%02x\n", chipID[0], chipID[1]);
			return 0;
		}
		cnt++;
						...
						int gpio_irq)
{
  ...
  {
    if(shtps_wait_tp_to_valid(ts)<0){
      result = 0;
      SHTPS_LOG_ERR_PRINT("shtps:CHIP ID read fail\n");
      shtps_mutex_unlock_ctrl();
      goto fail_irq_request;
    }
  }
