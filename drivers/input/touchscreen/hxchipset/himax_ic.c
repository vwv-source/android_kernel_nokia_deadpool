...
void self_test_output(uint8_t *RB1H, uint16_t *mutual_bank, uint16_t *self_bank)
{
    ...
    if (RB1H[0] == 0xAA)
    {
        ret += snprintf(output_buffer + ret, len - ret, "Self_Test Pass\n");
    }
    else if (RB1H[0] == 0xF1)
    {
        ret += snprintf(output_buffer + ret, len - ret, "Bank Average Fail\n");
    }
    else if (RB1H[0]==0xF2)
    {
        ret += snprintf(output_buffer + ret, len - ret, "Mutual block's bank Fail\n");
    }
    else if (RB1H[0]==0xF3)
    {
        ret += snprintf(output_buffer + ret, len - ret, "Self block's bank Fail\n");
    }
    ...
    if (!IS_ERR(fn)) {
        ...
        vfs_write(fn, output_buffer, 3*ret*sizeof(uint8_t), &pos);
        filp_close(fn, NULL);
    }
...
void himax_set_SMWP_enable(struct i2c_client *client, uint8_t SMWP_enable, bool suspended)
{
    uint8_t buf[4];
	uint8_t retry_cnt = 0;
	uint8_t smwp_en = 0x20;
	uint8_t smwp_dis = 0x00;
	uint8_t back_data = 0;

  buf[0] = (SMWP_enable == 1 && suspended) ? smwp_en : smwp_dis;
	back_data = buf[0];
	do {
    if ( i2c_himax_write(client, 0x8F,buf, 1, DEFAULT_RETRY_CNT) < 0)
        E("%s i2c write fail.\n",__func__);
		...
		i2c_himax_read(client, 0x8F, buf, 1, DEFAULT_RETRY_CNT);
		retry_cnt++;
	} while ((buf[0] != back_data ) && retry_cnt < 3);
}
...
int himax_read_Sensor_ID(struct i2c_client *client)
{
