...
extern int touch_double_tap_read(void);
extern int touch_double_tap_write(int);
extern struct fih_touch_cb touch_cb;
int Check_LCM_Staus = 1;
...
void calcDataSize(uint8_t finger_num)
{
...
{
	I("%s:Entering!\n", __func__);
	// if (himax_fw_ver_bin() == 0) {
	if (himax_fw_ver_bin(i_CTPM_FW) == 0) {
		I("%s:fw_ver in IC = %X in Bin = %X\n", __func__, ic_data->vendor_fw_ver ,g_i_FW_VER);
		I("%s:config_ver in IC = %X in Bin = %X\n", __func__, ic_data->vendor_config_ver, g_i_CFG_VER);
		...
		if (((ic_data->vendor_fw_ver != g_i_FW_VER) || (ic_data->vendor_config_ver != g_i_CFG_VER) ||
			(ic_data->vendor_cid_maj_ver != g_i_CID_MAJ) || (ic_data->vendor_cid_min_ver != g_i_CID_MIN))) {
			I("Need to update!\n");
			return NO_ERR;
		}	else {
			E("No need to update!\n");
...
static int i_get_FW(void)
	...
	/* Please add case if have new panel */
	if (ic_data->vendor_sensor_id == 0x11) {
		memcpy(firmware_name, "Himax_firmware_eag_truly.bin", sizeof(char)*strlen("Himax_firmware_eag_truly.bin"));
	} else if (ic_data->vendor_sensor_id == 0x12) {
		memcpy(firmware_name, "Himax_firmware_eag_12.bin", sizeof(char)*strlen("Himax_firmware_eag_12.bin"));
	} else if (ic_data->vendor_sensor_id == 0x14) {
		memcpy(firmware_name, "Himax_firmware_eag_14.bin", sizeof(char)*strlen("Himax_firmware_eag_14.bin"));
		...
		memcpy(firmware_name, "Himax_firmware_eag_22.bin", sizeof(char)*strlen("Himax_firmware_eag_22.bin"));
	} else if (ic_data->vendor_sensor_id == 0x24) {
		memcpy(firmware_name, "Himax_firmware_eag_24.bin", sizeof(char)*strlen("Himax_firmware_eag_24.bin"));
	} else if (ic_data->vendor_sensor_id == 0x41) {
		memcpy(firmware_name, "Himax_firmware_eag_41.bin", sizeof(char)*strlen("Himax_firmware_eag_41.bin"));
	} else if (ic_data->vendor_sensor_id == 0x42) {
		memcpy(firmware_name, "Himax_firmware_eag_42.bin", sizeof(char)*strlen("Himax_firmware_eag_42.bin"));
	...
	if (image != NULL) {
		...
		I("%s: i_CTPM_FW_len = %d\n", __func__, i_CTPM_FW_len);
		I("%s: i_CTPM_FW [0] = %02X, [1] = %02X, [2] = %02X, [3] = %02X\n", __func__
		, i_CTPM_FW[0], i_CTPM_FW[1], i_CTPM_FW[2], i_CTPM_FW[3]);
	} else {
		I("%s: i_CTPM_FW = NULL\n", __func__);
		return OPEN_FILE_FAIL;
    }
...
update_retry:
	...
	if (i_CTPM_FW_len == FW_SIZE_32k) {
		ret = fts_ctpm_fw_upgrade_with_sys_fs_32k(ts->client, i_CTPM_FW, i_CTPM_FW_len, false);
	} else if (i_CTPM_FW_len == FW_SIZE_60k) {
		ret = fts_ctpm_fw_upgrade_with_sys_fs_60k(ts->client, i_CTPM_FW, i_CTPM_FW_len, false);
	} else if (i_CTPM_FW_len == FW_SIZE_64k) {
		ret = fts_ctpm_fw_upgrade_with_sys_fs_64k(ts->client, i_CTPM_FW, i_CTPM_FW_len, false);
	} else if (i_CTPM_FW_len == FW_SIZE_124k) {
		ret = fts_ctpm_fw_upgrade_with_sys_fs_124k(ts->client, i_CTPM_FW, i_CTPM_FW_len, false);
	} else if (i_CTPM_FW_len == FW_SIZE_128k) {
		ret = fts_ctpm_fw_upgrade_with_sys_fs_128k(ts->client, i_CTPM_FW, i_CTPM_FW_len, false);
    }
...
#else
	himax_sense_on(ts->client, 0x00);
#endif
    if (ret != 0) {
		himax_sense_off(ts->client);
        himax_read_FW_ver(ts->client);
        himax_touch_information(ts->client);
		himax_sense_on(ts->client, 0x00);
		ts->x_channel = ic_data->HX_RX_NUM;
		ts->y_channel = ic_data->HX_TX_NUM;

		if (ts->nFinger_support != ic_data->HX_MAX_PT) {
...
static void himax_update_register(struct work_struct *work)
{
	...
	if (i_get_FW() != 0)
	{
		...
		if (g_f_boot_finish == 0) {
			...
			himax_set_SMWP_enable(private_ts->client, private_ts->SMWP_enable, private_ts->suspended);
			g_f_boot_finish = 1;
		}
#endif
...
int himax_chip_common_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_auto_update_flag = himax_calculateChecksum(client, false);
...
FW_force_upgrade:
	...
	//INIT_WORK(&ts->work_update, himax_update_register);
...
#endif
    //20180904    add for suspend to release touch point
    himax_int_enable(ts->client->irq,0);
