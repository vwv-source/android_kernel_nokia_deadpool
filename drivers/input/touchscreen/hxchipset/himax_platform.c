...
static int himax_regulator_configure(struct i2c_client *client,struct himax_i2c_platform_data *pdata)
{
    ...
    pdata->vcc_i2c = regulator_get(&client->dev,
                                   "vcc_i2c");
    if (IS_ERR(pdata->vcc_i2c))
    {
        E("%s: Failed to get regulator vcc_i2c\n",
          __func__);
        retval = PTR_ERR(pdata->vcc_i2c);
    }

    pdata->vcc_ana = regulator_get(&client->dev,"vcc_ana");
    if (IS_ERR(pdata->vcc_ana))
    {
        E("%s: Failed to get regulator vcc_ana\n",
          __func__);
...
int himax_power_on(struct himax_i2c_platform_data *pdata, bool on)
{
    ...
    if (on)
    {
        if(pdata->vcc_i2c)
        {
            retval = regulator_enable(pdata->vcc_i2c);
            if (retval)
            {
...
static int __init himax_common_init(void)
{
    ...
    if(i2c_add_driver(&himax_common_driver) < 0) {
