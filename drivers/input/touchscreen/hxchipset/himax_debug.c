...
#endif
//FIH touch -- start
char ResultSelfTest = 1; // FIH self test result
...
static int himax_diag_seq_read(struct seq_file *s, void *v)
{
    ...
    if((g_diag_command >= 1 && g_diag_command <= 7) || dsram_type > 0)
    {
        ...
        himax_get_self_edge();
        seq_printf(s, "Mutual Max:%d, Min:%d\n",g_max_mutual,g_min_mutual);
        seq_printf(s, "Self Max:%d, Min:%d\n",g_max_self,g_min_self);
...
firmware_upgrade_done:
	...
	private_ts->x_channel = ic_data->HX_RX_NUM;
	private_ts->y_channel = ic_data->HX_TX_NUM;

	if (private_ts->nFinger_support != ic_data->HX_MAX_PT) {
		...
		himax_input_register(private_ts);
	}
	g_f_hx_fw_update = 0;
	himax_int_enable(private_ts->client->irq,1);
...
char *g_hx_crtra_file = NULL;      //"hx_criteria.csv"
char *g_test_ok_file = NULL;       //"/data/misc/touch/Test_Data_OK.csv"
char *g_test_ng_file = NULL;       //"/data/misc/touch/Test_Data_NG.csv"
char *g_sensor_result_file = NULL; //"/data/misc/touch/sensortest_result.txt"
								...
								size_t len, loff_t *pos)
{
	...
	/* Free memory start */
	if (g_hx_crtra_file != NULL)
	{
		kfree(g_hx_crtra_file);
		g_hx_crtra_file = NULL;
	}
	if (g_test_ok_file != NULL)
	{
		kfree(g_test_ok_file);
		g_test_ok_file = NULL;
	}
	if (g_test_ng_file != NULL)
	{
		kfree(g_test_ng_file);
		g_test_ng_file = NULL;
	}
	if (g_sensor_result_file != NULL)
	{
		kfree(g_sensor_result_file);
		g_sensor_result_file = NULL;
	}
	/* Free memory end */
	...
	while ((token = strsep(&cur, " ")) != NULL)
	{
		...
		if (position == 0)
		{
			memset(criteria_name, 0x00, sizeof(criteria_name));
			/* Please modify criteria name */
			if (ic_data->vendor_sensor_id == 0x11) {
				memcpy(criteria_name, "hx_criteria_truly.csv", sizeof(char)*strlen("hx_criteria_truly.csv"));
			} else if (ic_data->vendor_sensor_id == 0x12) {
				memcpy(criteria_name, "hx_criteria_12.csv", sizeof(char)*strlen("hx_criteria_12.csv"));
			} else if (ic_data->vendor_sensor_id == 0x14) {
				memcpy(criteria_name, "hx_criteria_14.csv", sizeof(char)*strlen("hx_criteria_14.csv"));
			} else if (ic_data->vendor_sensor_id == 0x21) {
				memcpy(criteria_name, "hx_criteria_ld.csv", sizeof(char)*strlen("hx_criteria_ld.csv"));
			} else if (ic_data->vendor_sensor_id == 0x22) {
				memcpy(criteria_name, "hx_criteria_22.csv", sizeof(char)*strlen("hx_criteria_22.csv"));
			} else if (ic_data->vendor_sensor_id == 0x24) {
				memcpy(criteria_name, "hx_criteria_24.csv", sizeof(char)*strlen("hx_criteria_24.csv"));
			} else if (ic_data->vendor_sensor_id == 0x41) {
				memcpy(criteria_name, "hx_criteria_41.csv", sizeof(char)*strlen("hx_criteria_41.csv"));
			} else if (ic_data->vendor_sensor_id == 0x42) {
				memcpy(criteria_name, "hx_criteria_42.csv", sizeof(char)*strlen("hx_criteria_42.csv"));
			} else if (ic_data->vendor_sensor_id == 0x44) {
				memcpy(criteria_name, "hx_criteria_44.csv", sizeof(char)*strlen("hx_criteria_44.csv"));
			} else {
				E("%s: No such sensor_id please check !\n", __func__);
				goto fail;
			}

			g_hx_crtra_file = kzalloc((sizeof(char)*(strlen(criteria_name)+1)), GFP_KERNEL);
			if (g_hx_crtra_file == NULL)
			{
				E("%s: Memory allocate fail for criteria !\n", __func__);
				goto fail;
			}
			memcpy(g_hx_crtra_file, criteria_name, (sizeof(char)*(strlen(criteria_name)+1)));
			I("g_hx_crtra_file=%s\n", g_hx_crtra_file);
		}
		else if (position == 1)
		{
			g_test_ok_file = kzalloc(token_len, GFP_KERNEL);
			if (g_test_ok_file == NULL)
			{
				E("%s: Memory allocate fail for test_ok_file !\n", __func__);
				goto fail;
			}
			memcpy(g_test_ok_file, token, token_len);
			I("g_test_ok_file=%s\n", g_test_ok_file);
		}
		else if (position == 2)
		{
			g_test_ng_file = kzalloc(token_len, GFP_KERNEL);
			if (g_test_ng_file == NULL)
			{
				E("%s: Memory allocate fail for test_ng_file !\n", __func__);
				goto fail;
			}
			memcpy(g_test_ng_file, token, token_len);
			I("g_test_ng_file=%s\n", g_test_ng_file);
		}
		else if (position == 3)
		{
			g_sensor_result_file = kzalloc(token_len, GFP_KERNEL);
			if (g_sensor_result_file == NULL)
			{
			...
			if (*(token+token_len-1) == 0x0A)
				memcpy(g_sensor_result_file, token, token_len-1);
			else
			{
				E("%s: Fail command!\n", __func__);
				goto fail;
			}
			I("g_sensor_result_file=%s\n", g_sensor_result_file);
		}
		position++;
	}
                                    ...
                                    size_t len, loff_t *pos)
{
	...
	if ((g_hx_crtra_file == NULL) || (g_test_ok_file == NULL) ||
		(g_test_ng_file == NULL) || (g_sensor_result_file == NULL))
	{
    ...
    if(!HX_PROC_SEND_FLAG)
    {
        ...
        {
            ret += snprintf(temp_buf + ret, len - ret, "Bank Average Fail\n");
        }
        else if (val == 0x02)
        {
            ret += snprintf(temp_buf + ret, len - ret, "Mutual block's bank Fail\n");
        }
        else if (val == 0x03)
        {
            ret += snprintf(temp_buf + ret, len - ret, "Self block's bank Fail\n");
        }
        else
        {
                                ...
                                size_t len, loff_t *pos)
{
    ...
    HX_SMWP_EN_eag = ts->SMWP_enable;
    I("%s: SMART_WAKEUP_enable = %d.\n", __func__, HX_SMWP_EN_eag);
	mutex_unlock(&private_ts->himax_mutex);
    return len;
}
...
#ifdef HX_AUTO_UPDATE_FW
static ssize_t himax_proc_fw_upgrade_write(struct file *file, const char *buff,
        size_t len, loff_t *pos)
{
    struct himax_ts_data *ts = private_ts;
    char buf_tmp[20];
    ...
    if (buf_tmp[0] == '1') {
        I("Update fw with comparing version\n");
        queue_delayed_work(ts->himax_update_wq, &ts->work_update, 0);
        //queue_work(ts->himax_update_wq, &ts->work_update);
    } else if (buf_tmp[0] == '2') {
        I("Update fw without comparing version\n");
...
{
    .owner = THIS_MODULE,
    .write = himax_proc_fw_upgrade_write,
};
#endif
...
#ifdef HX_SMART_WAKEUP
    himax_proc_SMWP_file_eag = proc_create(HIMAX_PROC_SMWP_FILE_EAG, (S_IWUSR|S_IRUGO|S_IWUGO),
                                       himax_touch_proc_dir, &himax_proc_SMWP_ops);
    if(himax_proc_SMWP_file_eag == NULL)
    {
        E(" %s: proc SMWP file create failed!\n", __func__);
        goto fail_14;
    }
    himax_proc_GESTURE_file_eag = proc_create(HIMAX_PROC_GESTURE_FILE_EAG, (S_IWUSR|S_IRUGO|S_IWUGO),
                                          himax_touch_proc_dir, &himax_proc_Gesture_ops);
    if(himax_proc_GESTURE_file_eag == NULL)
    {
...
#endif
#ifdef HX_AUTO_UPDATE_FW
	himax_proc_fw_upgrade_file = proc_create(HIMAX_PROC_FW_UPGRADE_FILE, (S_IWUSR|S_IRUGO|S_IWUGO),
...
#ifdef HX_AUTO_UPDATE_FW
	remove_proc_entry(HIMAX_PROC_FW_UPGRADE_FILE, himax_touch_proc_dir);
fail_20:
#endif
#ifdef HX_TP_PROC_GUEST_INFO
fail_19:
...
#endif
#ifdef HX_SMART_WAKEUP
    remove_proc_entry( HIMAX_PROC_GESTURE_FILE_EAG, himax_touch_proc_dir );
fail_15:
    remove_proc_entry( HIMAX_PROC_SMWP_FILE_EAG, himax_touch_proc_dir );
fail_14:
#endif
#ifdef HX_HIGH_SENSE
...
void touch_vendor_read(char *buf)
{
    ...
    memset(cmd_d2, 0x00, sizeof(cmd_d2));
    himax_ic_reset(false,false);
    if (i2c_himax_read(private_ts->client, 0xD1, cmd, 3, 3) < 0)
        return;
            ...
            (cmd[2] == 0x25 || cmd[2] == 0x26 || cmd[2] == 0x27 || cmd[2] == 0x28))
    {
        ...
        I("Himax IC package cmd[0]=%x,cmd[1]=%x,cmd[2]=%x\n",cmd[0],cmd[1],cmd[2]);
    }

    msleep(50);
    if (i2c_himax_read(private_ts->client, 0xD2, cmd_d2, 4, 3) < 0)
        return;
    himax_sense_on(private_ts->client,0);
    I("Himax IC package D2 register = %x\n",cmd_d2[3]);
    sprintf(buf, "%s-%x%x%c%x\n","Himax",cmd[1],cmd[2],chip_serial[0],cmd_d2[3]);
}
...
int selftest_result_read(void)
{
    ...
    if (ResultSelfTest == 0)
        result = 0;
    else if (ResultSelfTest == 1)
        result = 1;
    else if (ResultSelfTest == 2)
        result = 2;
    else if (ResultSelfTest == 3)
        result = 3;
    return result;
}
...
{
    I("%s, double_tap_enable = %d", __func__, double_tap_enable);

    return double_tap_enable;
}

int touch_double_tap_write(int enable)
{
#ifdef HX_SMART_WAKEUP
    struct himax_ts_data *ts = private_ts;
    I("%s: set double_tap_enable = %d", __func__, enable);
    if (!Check_LCM_Staus)
    {
    ...
    else
    {
        ...
        if(g_f_boot_finish == 1) {
            I("%s: g_f_boot_finish == 1!\n", __func__);
            mutex_lock(&private_ts->himax_mutex);
            I("%s: mutex !\n", __func__);
            himax_set_SMWP_enable(ts->client, ts->SMWP_enable, ts->suspended);
            mutex_unlock(&private_ts->himax_mutex);
        ...
        double_tap_enable = ts->SMWP_enable;

        I("%s: END!\n", __func__);
    }
#endif
    I("%s: double_tap_enable = enable = %d\n", __func__, double_tap_enable);

    return 0;
}
/* mutex sample code by Taipei FIH
