...
static int gtp_gesture_handler(struct goodix_ts_data *ts)
{
    ...
    if ((doze_buf[2] == 'a') || (doze_buf[2] == 'b') ||
        ...
        (doze_buf[2] == 0xCC)) {
        dev_info(&ts->client->dev, "[FIH@touch]0x814B = 0x%02X enter gesture input key event\n", doze_buf[2]);
...
s32 fih_gtp_read_version(struct i2c_client *client, u16* version)
{
    ...
    if(fw_info->sensor_id == CTC_ID){
        snprintf(fih_touch_gt9xx, PAGE_SIZE, "Goodix-V%02x%02x_C%d\n", buf[7], buf[6], buf_a[2]);
    }else if(fw_info->sensor_id == HLT_ID){
        snprintf(fih_touch_gt9xx, PAGE_SIZE, "Goodix-V%02x%02x_H%d\n", buf[7], buf[6], buf_a[2]);
    }else if(fw_info->sensor_id == INO_ID){
        snprintf(fih_touch_gt9xx, PAGE_SIZE, "Goodix-V%02x%02x_I%d\n", buf[7], buf[6], buf_a[2]);
    }else {
        snprintf(fih_touch_gt9xx, PAGE_SIZE, "Goodix-V%02x%02x_%d\n", buf[7], buf[6], buf_a[2]);
    }
            ...
            struct goodix_ts_platform_data *pdata)
{
    ...
    dev_info(dev, "[FIH@touch] devinfo =%d\n",pdata->devinfo);
...
static int gtp_power_on(struct goodix_ts_data *ts)
{
    ...
    if (ts->vdd_ana) {
        ...
        ret = regulator_set_voltage(ts->vdd_ana, 3000000, 3000000); //FIH, Set AVDD as 3.3V
...
static void gtp_suspend(struct goodix_ts_data *ts)
{
    ...
    if (ts->pdata->slide_wakeup && gdouble_tap_enable_gt9xx) {
        dev_info(&ts->client->dev, "enter gesture mode\n");
...
static void fb_notify_resume_work(struct work_struct *work)
{
    ...
    pr_info("F@TOUCH %s try resume in workqueue \n",__func__);
    gtp_resume(ts);
}
    ...
    if (ev_data && ev_data->data && event == FB_EARLY_EVENT_BLANK && ts) {
        ...
        if (*blank == FB_BLANK_UNBLANK ||
            *blank == FB_BLANK_NORMAL) {
            dev_info(&ts->client->dev, "F@TOUCH %s FB_BLANK_UNBLANK, RESUME by fb notifier HERE!\n",__func__);
            gLcmOnOff = 1;
            ...
            dev_info(&ts->client->dev, "F@TOUCH %s FB_BLANK_POWERDOWN, SUSPEND by fb notifier HERE!\n",__func__);
