...
#define GOODIX_FIRMWARE_FILE_NAME_INO         "goodix_firmware_ino.bin"
#define GOODIX_CONFIG_FILE_NAME_INO           "goodix_config_ino.cfg"
#define GOODIX_FIRMWARE_FILE_NAME_HLT         "goodix_firmware_hlt.bin"
#define GOODIX_CONFIG_FILE_NAME_HLT           "goodix_config_hlt.cfg"
#define GOODIX_FIRMWARE_FILE_NAME_CTC         "goodix_firmware_ctc.bin"
#define GOODIX_CONFIG_FILE_NAME_CTC           "goodix_config_ctc.cfg"
...
static int gup_update_config(struct i2c_client *client)
{
    ...
    if(ts->fw_info.sensor_id == INO_ID){//EAG firmware upgrade is here
        ret = request_firmware(&fw_cfg, GOODIX_CONFIG_FILE_NAME_INO,&client->dev);
        pr_info("[FIH@touch] config file - %s \n",GOODIX_CONFIG_FILE_NAME_INO);
    ...
    }else if(ts->fw_info.sensor_id == CTC_ID){//EAG firmware upgrade is here
        ret = request_firmware(&fw_cfg, GOODIX_CONFIG_FILE_NAME_CTC,&client->dev);
        pr_info("[FIH@touch] config file - %s \n",GOODIX_CONFIG_FILE_NAME_CTC);
    }else {
        ...
        u8 **path_p)
{
    ...
    if (!(*path_p)) {
        if(ts->fw_info.sensor_id == INO_ID)
            *path_p = GOODIX_FIRMWARE_FILE_NAME_INO;
        else if(ts->fw_info.sensor_id == HLT_ID){
            if(ts->pdata->devinfo == DEV_CTL_HLT14){
             ...
             *path_p = GOODIX_FIRMWARE_FILE_NAME_CTC;
        else {
...
s32 gup_update_proc(void *dir)
{
    ...
    dev_err(&ts->client->dev, "[update_proc]Begin update ......\n");
    ...
    if (ts->force_update || dir) {
        dev_err(&ts->client->dev, "Enter force update.");
    } else {
...
update_fail:
    dev_err(&ts->client->dev, "[update_proc]leave update mode.");
