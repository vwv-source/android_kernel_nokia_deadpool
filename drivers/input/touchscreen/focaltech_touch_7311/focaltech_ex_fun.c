...
void touch_tpfwver_read_focalTP(char *fw_ver)
{
    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev = ts_data->input_dev;
    struct i2c_client *client = ts_data->client;
...
void touch_tpfwimver_read_focalTP(char *fw_ver)
{
}

static int fw_upgrade_flag = 0;
int gForceUpgrade = 0;
void touch_fwupgrade_focalTP(int input)
{
    char fwname[FILE_NAME_LENGTH];
    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev = ts_data->input_dev;
    struct i2c_client *client = ts_data->client;
     ...
     //FIH, If set focaltech,fw-auto-upgrade-disable in device tree, skipp upgrade function
     /*   if((input<2) && fts_wq_data->pdata->auto_upgrade_disable){
            FTS_ERROR("TP skip upgrade");
            return;
        }
     */

    memset(fwname, 0, sizeof(fwname));
    strncpy(fwname, fw_app_bin, FILE_NAME_LENGTH);
    fwname[FILE_NAME_LENGTH - 1] = '\0';
    ...
    fw_upgrade_flag = fts_upgrade_bin(client, fwname, 0);
...
void touch_fwupgrade_read_focalTP(char *buf)
{
    sprintf(buf, "%d\n", fw_upgrade_flag);
}

void touch_vendor_read_focalTP(char *buf)
{
    sprintf(buf, "%s\n","FocalTech");
}
