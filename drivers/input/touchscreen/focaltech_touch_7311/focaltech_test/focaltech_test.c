...
bool ResultSelfTest_focalTP = false; // FIH self test result
/*****************************************************************************
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    sprintf(filepath, "%s%s", FTS_TEST_DATA_PATH, file_name);
    if (NULL == pfile) {
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    if (true == fts_test_start()) {
        FTS_TEST_SAVE_INFO("\n=======Tp test pass. \n\n");
        ResultSelfTest_focalTP = true;
    } else {
        FTS_TEST_SAVE_INFO("\n=======Tp test failure. \n\n");
        ResultSelfTest_focalTP = false;
    }
...
void touch_selftest_focalTP(void)
{
    char fwname[128] = {0};
    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev;
...
int selftest_result_read_focalTP(void)
{
    int num_read_chars = 0;
    if(ResultSelfTest_focalTP)
        num_read_chars = 0;
    else
        num_read_chars = 1;
