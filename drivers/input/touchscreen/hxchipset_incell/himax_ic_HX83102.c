...
{
	private_incell_ts->chip_cell_type = CHIP_IS_IN_CELL;
	I("%s:IC cell type = %d\n", __func__, private_incell_ts->chip_cell_type);
	INCELL_IC_CHECKSUM 			= HX_TP_BIN_CHECKSUM_CRC;
	/*Himax: Set FW and CFG Flash Address*/
	INCELL_FW_VER_MAJ_FLASH_ADDR   = 49157;  /*0x00C005*/
	INCELL_FW_VER_MAJ_FLASH_LENG   = 1;
	INCELL_FW_VER_MIN_FLASH_ADDR   = 49158;  /*0x00C006*/
	INCELL_FW_VER_MIN_FLASH_LENG   = 1;
	INCELL_CFG_VER_MAJ_FLASH_ADDR 	= 49408;  /*0x00C100*/
	INCELL_CFG_VER_MAJ_FLASH_LENG 	= 1;
	INCELL_CFG_VER_MIN_FLASH_ADDR 	= 49409;  /*0x00C101*/
	INCELL_CFG_VER_MIN_FLASH_LENG 	= 1;
	INCELL_CID_VER_MAJ_FLASH_ADDR	= 49154;  /*0x00C002*/
	INCELL_CID_VER_MAJ_FLASH_LENG	= 1;
	INCELL_CID_VER_MIN_FLASH_ADDR	= 49155;  /*0x00C003*/
	INCELL_CID_VER_MIN_FLASH_LENG	= 1;
...
static bool hx83102_chip_detect(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x10) && ((tmp_data[1] == 0x2a) || (tmp_data[1] == 0x2b) || (tmp_data[1] == 0x2d))) {
			if (tmp_data[1] == 0x2a) {
				...
				strlcpy(private_incell_ts->chip_name, HX_83102B_SERIES_PWON, 30);
				I("%s:detect IC HX83102B successfully\n", __func__);
			} else {
				strlcpy(private_incell_ts->chip_name, HX_83102D_SERIES_PWON, 30);
	...
	incell_ic_data->vendor_ic_ver = 0xFFFF;
	if (ret_data == true) {
		/* Enable read DD */
		himax_in_parse_assign_cmd(hx83102d_ic_osc_pw, tmp_addr, 4);
		g_core_fp.fp_register_read(tmp_addr, FOUR_BYTE_DATA_SZ, tmp_data, 0);
		tmp_data[0] = 0xA5;
		g_core_fp.fp_register_write(tmp_addr,  FOUR_BYTE_DATA_SZ,  tmp_data,  false);
		
		himax_in_parse_assign_cmd(hx83102d_ic_osc_en, tmp_addr, 4);
		g_core_fp.fp_register_read(tmp_addr, FOUR_BYTE_DATA_SZ, tmp_data, 0);
		tmp_data[0] = 0xDD;
		g_core_fp.fp_register_write(tmp_addr,  FOUR_BYTE_DATA_SZ,  tmp_data,  false);

		himax_in_parse_assign_cmd(hx83102d_ic_b9_en, tmp_addr, 4);
		tmp_data[0] = 0x00;
		tmp_data[1] = 0x83;
		tmp_data[2] = 0x10;
		tmp_data[3] = 0x2D;
		g_core_fp.fp_register_write(tmp_addr,  FOUR_BYTE_DATA_SZ,  tmp_data,  false);
		/* Read DD data */
		himax_in_parse_assign_cmd(hx83102d_ic_fw, tmp_addr, 4);
		g_core_fp.fp_register_read(tmp_addr, FOUR_BYTE_DATA_SZ, tmp_data, 0);
		I("tmp_data[0]=%X\n", tmp_data[0]);
		incell_ic_data->vendor_ic_ver = tmp_data[0] & 0x0F;
		/* Disable read DD */
		himax_in_parse_assign_cmd(hx83102d_ic_b9_en, tmp_addr, 4);
		tmp_data[0] = 0x00;
		tmp_data[1] = 0x00;
	...
	I("VENDOR_IC_VER : %X \n", (incell_ic_data->vendor_ic_ver));

	if (incell_i_CTPM_firmware_name != NULL) {
		kfree(incell_i_CTPM_firmware_name);
		incell_i_CTPM_firmware_name = NULL;
	}
	memset(firmware_name, 0x00, sizeof(firmware_name));

	if (spi_dev_detected == true) { /* INX */
		if ((incell_ic_data->vendor_ic_ver == 0x00) || (incell_ic_data->vendor_ic_ver == 0x21)) {
			memcpy(firmware_name, "Himax_firmware_inx_1.bin", sizeof(char)*strlen("Himax_firmware_inx_1.bin"));
		} else {
			memcpy(firmware_name, "Himax_firmware_inx_2.bin", sizeof(char)*strlen("Himax_firmware_inx_2.bin"));
		}
	} else { /* AUO */
		if ((incell_ic_data->vendor_ic_ver == 0x00) || (incell_ic_data->vendor_ic_ver == 0x21)) {
			memcpy(firmware_name, "Himax_firmware_auo_1.bin", sizeof(char)*strlen("Himax_firmware_auo_1.bin"));
		} else {
			memcpy(firmware_name, "Himax_firmware_auo_2.bin", sizeof(char)*strlen("Himax_firmware_auo_2.bin"));
		}	
	}

	incell_i_CTPM_firmware_name = kzalloc((sizeof(char)*(strlen(firmware_name)+1)), GFP_KERNEL);
	if (incell_i_CTPM_firmware_name != NULL) {
		memcpy(incell_i_CTPM_firmware_name, firmware_name, (sizeof(char)*(strlen(firmware_name)+1)));
	}
	I("incell_i_CTPM_firmware_name : %s \n", incell_i_CTPM_firmware_name);
