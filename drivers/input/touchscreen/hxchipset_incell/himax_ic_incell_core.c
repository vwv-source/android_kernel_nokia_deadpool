...
#include "himax_ic_core.h"
	...
	extern int incell_g_i_FW_VER;
	extern int incell_g_i_CFG_VER;
	extern int incell_g_i_CID_MAJ;
	extern int incell_g_i_CID_MIN;
	extern unsigned char *incell_i_CTPM_FW;
#endif
#ifdef HX_ZERO_FLASH
extern int g_f_0f_updat;
#endif

extern unsigned long INCELL_FW_VER_MAJ_FLASH_ADDR;
extern unsigned long INCELL_FW_VER_MIN_FLASH_ADDR;
extern unsigned long INCELL_CFG_VER_MAJ_FLASH_ADDR;
extern unsigned long INCELL_CFG_VER_MIN_FLASH_ADDR;
extern unsigned long INCELL_CID_VER_MAJ_FLASH_ADDR;
extern unsigned long INCELL_CID_VER_MIN_FLASH_ADDR;

extern unsigned long INCELL_FW_VER_MAJ_FLASH_LENG;
extern unsigned long INCELL_FW_VER_MIN_FLASH_LENG;
extern unsigned long INCELL_CFG_VER_MAJ_FLASH_LENG;
extern unsigned long INCELL_CFG_VER_MIN_FLASH_LENG;
extern unsigned long INCELL_CID_VER_MAJ_FLASH_LENG;
extern unsigned long INCELL_CID_VER_MIN_FLASH_LENG;
...
extern int g_incell_zero_event_count;
#endif

#ifdef HX_RST_PIN_FUNC
	extern u8 INCELL_HX_HW_RESET_ACTIVATE;
	extern void himax_incell_rst_gpio_set(int pinnum, uint8_t value);
#endif
...
extern int himax_incell_report_data_init(void);
...
static int himax_mcu_register_read(uint8_t *read_addr, uint32_t read_length, uint8_t *read_data, uint8_t cfg_flag)
{
	...
	if (cfg_flag == false) {
		if (spi_dev_detected)
			flash_r_max_len = FLASH_RW_MAX_LEN*16; /* Max 4K */
		else
			flash_r_max_len = FLASH_RW_MAX_LEN;

		if (read_length > flash_r_max_len) {
			E("%s: read len over %d!\n", __func__, flash_r_max_len);
			return LENGTH_FAIL;
		}
...
#ifdef CORE_FW
/* FW side start*/
static void diag_mcu_parse_raw_data(struct himax_report_data *hx_incell_touch_data, int mul_num, int self_num, uint8_t diag_cmd, int32_t *mutual_data, int32_t *self_data)
{
	int RawDataLen_word;
	int index = 0;
	int temp1, temp2, i;

	if (hx_incell_touch_data->hx_rawdata_buf[0] == pfw_op->data_rawdata_ready_lb[0]
	    && hx_incell_touch_data->hx_rawdata_buf[1] == pfw_op->data_rawdata_ready_hb[0]
	    && hx_incell_touch_data->hx_rawdata_buf[2] > 0
	    && hx_incell_touch_data->hx_rawdata_buf[3] == diag_cmd) {
		RawDataLen_word = hx_incell_touch_data->rawdata_size / 2;
		index = (hx_incell_touch_data->hx_rawdata_buf[2] - 1) * RawDataLen_word;
		...
		for (i = 0; i < RawDataLen_word; i++) {
			...
			if (temp1 < mul_num) { /*mutual*/
				mutual_data[index + i] = ((int8_t)hx_incell_touch_data->hx_rawdata_buf[i * 2 + 4 + 1]) * 256 + hx_incell_touch_data->hx_rawdata_buf[i * 2 + 4];
			} else { /*self*/
				temp1 = i + index;
				...
				self_data[i + index - mul_num] = (((int8_t)hx_incell_touch_data->hx_rawdata_buf[i * 2 + 4 + 1]) << 8) +
				hx_incell_touch_data->hx_rawdata_buf[i * 2 + 4];
			}
		}
	}
}
...
static bool himax_mcu_check_chip_version(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x10) && (tmp_data[1] == 0x2a)) {
			strlcpy(private_incell_ts->chip_name, HX_83102A_SERIES_PWON, 30);
			ret_data = true;
			break;
		} else {
			ret_data = false;
...
static void himax_mcu_read_FW_ver(void)
{
	...
	while (reload_status == 0) {
		...
		if ((data[1] == 0x3A && data[0] == 0xA3)
			...
			incell_ic_data->vendor_panel_ver = 0;
			incell_ic_data->vendor_fw_ver = 0;
			incell_ic_data->vendor_config_ver = 0;
			incell_ic_data->vendor_touch_cfg_ver = 0;
			incell_ic_data->vendor_display_cfg_ver = 0;
			incell_ic_data->vendor_cid_maj_ver = 0;
			incell_ic_data->vendor_cid_min_ver = 0;
			return;
		} else {
			retry--;
			msleep(10);
	...
	incell_ic_data->vendor_panel_ver =  data[0];
	incell_ic_data->vendor_fw_ver = data[1] << 8 | data[2];
	...
	incell_ic_data->vendor_config_ver = data[2] << 8 | data[3];
	/*I("CFG_VER : %X \n",incell_ic_data->vendor_config_ver);*/
	incell_ic_data->vendor_touch_cfg_ver = data[2];
	I("TOUCH_VER : %X \n", incell_ic_data->vendor_touch_cfg_ver);
	incell_ic_data->vendor_display_cfg_ver = data[3];
	I("DISPLAY_VER : %X \n", incell_ic_data->vendor_display_cfg_ver);
	g_core_fp.fp_register_read(pfw_op->addr_fw_vendor_addr, FOUR_BYTE_DATA_SZ, data, 0);
	incell_ic_data->vendor_cid_maj_ver = data[2] ;
	incell_ic_data->vendor_cid_min_ver = data[3];
...
{
	if (switch_on) {
		if (private_incell_ts->use_irq) {
			himax_incell_int_enable(switch_on);
		} else {
			hrtimer_start(&private_incell_ts->timer, ktime_set(1, 0), HRTIMER_MODE_REL);
		}
	} else {
		if (private_incell_ts->use_irq) {
			himax_incell_int_enable(switch_on);
		} else {
			hrtimer_cancel(&private_incell_ts->timer);
			cancel_work_sync(&private_incell_ts->work);
		}
	}
}
...
static bool himax_mcu_get_DSRAM_data(uint8_t *info_data, bool DSRAM_Flag)
{
	...
	uint8_t x_num = incell_ic_data->HX_RX_NUM;
	uint8_t y_num = incell_ic_data->HX_TX_NUM;
	/*int m_key_num = 0;*/
	...
	/*m_key_num = incell_ic_data->HX_BT_NUM;
	I("%s,m_key_num=%d\n",__func__ ,m_key_num);
	...
	/* 4. FW stop outputing */
	/*I("DSRAM_Flag=%d\n",DSRAM_Flag);*/
	if (DSRAM_Flag == false || private_incell_ts->diag_cmd == 0) {
...
static int himax_mcu_fw_ver_bin(void)
{
	...
	I("%s:Entering!\n", __func__);
	if (incell_i_CTPM_FW != NULL) {
		I("Catch fw version in bin file!\n");
		incell_g_i_FW_VER = (incell_i_CTPM_FW[INCELL_FW_VER_MAJ_FLASH_ADDR] << 8) | incell_i_CTPM_FW[INCELL_FW_VER_MIN_FLASH_ADDR];
		incell_g_i_CFG_VER = (incell_i_CTPM_FW[INCELL_CFG_VER_MAJ_FLASH_ADDR] << 8) | incell_i_CTPM_FW[INCELL_CFG_VER_MIN_FLASH_ADDR];
		incell_g_i_CID_MAJ = incell_i_CTPM_FW[INCELL_CID_VER_MAJ_FLASH_ADDR];
		incell_g_i_CID_MIN = incell_i_CTPM_FW[INCELL_CID_VER_MIN_FLASH_ADDR];
	} else {
		I("FW data is null!\n");
		return 1;
	}
...
static void himax_mcu_pin_reset(void)
{
	...
	himax_incell_rst_gpio_set(private_incell_ts->rst_gpio, 0);
	msleep(20);
	himax_incell_rst_gpio_set(private_incell_ts->rst_gpio, 1);
	msleep(50);
}
...
static void himax_mcu_touch_information(void)
{
	...
	incell_ic_data->HX_RX_NUM				= data[2];
	incell_ic_data->HX_TX_NUM				= data[3];
	incell_ic_data->HX_MAX_PT				= data[4];
	/*I("%s : HX_RX_NUM=%d,incell_ic_data->HX_TX_NUM=%d,incell_ic_data->HX_MAX_PT=%d\n",__func__,incell_ic_data->HX_RX_NUM,incell_ic_data->HX_TX_NUM,incell_ic_data->HX_MAX_PT);*/
	...
	if ((data[1] & 0x04) == 0x04) {
		incell_ic_data->HX_XY_REVERSE = true;
	} else {
		incell_ic_data->HX_XY_REVERSE = false;
	}

	g_core_fp.fp_register_read(pdriver_op->addr_fw_define_x_y_res, FOUR_BYTE_DATA_SZ, data, 0);
	incell_ic_data->HX_Y_RES = data[0] * 256 + data[1];
	incell_ic_data->HX_X_RES = data[2] * 256 + data[3];
	/*I("%s : incell_ic_data->HX_Y_RES=%d,incell_ic_data->HX_X_RES=%d \n",__func__,incell_ic_data->HX_Y_RES,incell_ic_data->HX_X_RES);*/
	...
	I("data[0] & 0x01 = %d\n",(data[0] & 0x01));*/
	// if ((data[1] & 0x01) == 1) {
		incell_ic_data->HX_INT_IS_EDGE = true;
	// } else {
		// incell_ic_data->HX_INT_IS_EDGE = false;
	// }

	if (incell_ic_data->HX_RX_NUM > 40) {
		incell_ic_data->HX_RX_NUM = 32;
	}

	if (incell_ic_data->HX_TX_NUM > 20) {
		incell_ic_data->HX_TX_NUM = 18;
	}

	if (incell_ic_data->HX_MAX_PT > 10) {
		incell_ic_data->HX_MAX_PT = 10;
	}

	if (incell_ic_data->HX_Y_RES > 2000) {
		incell_ic_data->HX_Y_RES = 1280;
	}

	if (incell_ic_data->HX_X_RES > 2000) {
		incell_ic_data->HX_X_RES = 720;
	}
	...
	/* I("%s: tmp_data[0] = 0x%02X,tmp_data[1] = 0x%02X,tmp_data[2] = 0x%02X,tmp_data[3] = 0x%02X\n",
	 __func__, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);*/
	incell_ic_data->HX_BT_NUM = data[0] & 0x03;
#else
	incell_ic_data->HX_RX_NUM				= FIX_HX_RX_NUM;
	incell_ic_data->HX_TX_NUM				= FIX_HX_TX_NUM;
	incell_ic_data->HX_BT_NUM				= FIX_HX_BT_NUM;
	incell_ic_data->HX_X_RES				= FIX_HX_X_RES;
	incell_ic_data->HX_Y_RES				= FIX_HX_Y_RES;
	incell_ic_data->HX_MAX_PT				= FIX_HX_MAX_PT;
	incell_ic_data->HX_XY_REVERSE			= FIX_HX_XY_REVERSE;
	incell_ic_data->HX_INT_IS_EDGE			= FIX_HX_INT_IS_EDGE;
#endif
	I("%s:HX_RX_NUM =%d,HX_TX_NUM =%d,HX_MAX_PT=%d \n", __func__, incell_ic_data->HX_RX_NUM, incell_ic_data->HX_TX_NUM, incell_ic_data->HX_MAX_PT);
	I("%s:HX_XY_REVERSE =%d,HX_Y_RES =%d,HX_X_RES=%d \n", __func__, incell_ic_data->HX_XY_REVERSE, incell_ic_data->HX_Y_RES, incell_ic_data->HX_X_RES);
	I("%s:HX_INT_IS_EDGE =%d \n", __func__, incell_ic_data->HX_INT_IS_EDGE);
}

static void himax_mcu_reload_config(void)
{
	if (himax_incell_report_data_init()) {
...
static bool himax_mcu_diag_check_sum(struct himax_report_data *hx_incell_touch_data)
{
	uint16_t check_sum_cal = 0;
	int i;

	/* Check 128th byte CRC */
	for (i = 0, check_sum_cal = 0; i < (hx_incell_touch_data->touch_all_size - hx_incell_touch_data->touch_info_size); i += 2) {
		check_sum_cal += (hx_incell_touch_data->hx_rawdata_buf[i + 1] * FLASH_RW_MAX_LEN + hx_incell_touch_data->hx_rawdata_buf[i]);
	}
...
static void himax_mcu_diag_parse_raw_data(struct himax_report_data *hx_incell_touch_data, int mul_num, int self_num, uint8_t diag_cmd, int32_t *mutual_data, int32_t *self_data)
{
	diag_mcu_parse_raw_data(hx_incell_touch_data, mul_num, self_num, diag_cmd, mutual_data, self_data);
}
...
static int himax_mcu_ic_esd_recovery(int hx_esd_event, int hx_zero_event, int length)
{
	...
	if (g_incell_zero_event_count > 5) {
		//himax_mcu_register_read(tmp_addr, 4, tmp_data, false); 
...
void himax_mcu_firmware_update_0f(const struct firmware *fw_entry)
{
	...
	/*FW config*/
	if (G_POWERONOF == 1) {
		g_core_fp.fp_write_sram_0f(fw_entry, pzf_op->data_fw_cfg_1, 0xC0FE, 512);
	} else {
		g_core_fp.fp_clean_sram_0f(pzf_op->data_fw_cfg_1, 512, 1);
	}
	if (G_POWERONOF == 1) {
		g_core_fp.fp_write_sram_0f(fw_entry, pzf_op->data_fw_cfg_2, 0xC9DE, 164);
	} else {
		g_core_fp.fp_clean_sram_0f(pzf_op->data_fw_cfg_2, 164, 1);
	}
...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	incell_g_f_hx_fw_update = 1;
	I("%s, Entering \n", __func__);
	I("file name = %s\n", file_name);
	err = request_firmware (&fw_entry, file_name, private_incell_ts->dev);
	if (err < 0) {
		E("%s, fail in line%d error code=%d,file maybe fail\n", __func__, __LINE__, err);
		incell_g_f_hx_fw_update = 0;
...
int himax_mcu_0f_operation_dirly(void)
{
	...
	err = request_firmware (&fw_entry, incell_i_CTPM_firmware_name, private_incell_ts->dev);
	if (err < 0) {
		E("%s, fail in line%d error code=%d,file maybe fail\n", __func__, __LINE__, err);
		incell_g_f_hx_fw_update = 0;
		mutex_unlock(&private_incell_ts->himax_mutex);
...
void himax_mcu_firmware_read_0f(const struct firmware *fw_entry, int type)
{
	...
	if (type == 0) { /* first 48K */
		...
		/*FW config*/
		g_core_fp.fp_read_sram_0f(fw_entry, pzf_op->data_fw_cfg_1, 0xC0FE, 512);
		g_core_fp.fp_read_sram_0f(fw_entry, pzf_op->data_fw_cfg_2, 0xC9DE, 164);
