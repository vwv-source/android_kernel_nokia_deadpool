 ...
 *
 * $Revision: 20251 $
 * $Date: 2017-12-13 17:41:29 +0800 (?±‰?, 13 ?Å‰???2017) $
 *
...
//-----[20181012]Add Power Status Notify--------
...
int tpd_usb_plugin(int plugin)
{
	...
	pr_debug("[HL]%s, %d", __func__, __LINE__);
	...
	switch(plugin) {
        case USB_PLUGOUT:
            ...
            pr_debug("[HL]%s, %d: bTouchIsAwake = %d\n", __func__, __LINE__, bTouchIsAwake);
            if (!bTouchIsAwake)
            {
            	pr_debug("[HL]%s, %d: bTouchIsAwake is Off! Do Nothing!\n", __func__, __LINE__);
            	break;
            }
            else
            {
            	pr_debug("[HL]%s, %d: bTouchIsAwake is On! Keep Going!\n", __func__, __LINE__);
            }
