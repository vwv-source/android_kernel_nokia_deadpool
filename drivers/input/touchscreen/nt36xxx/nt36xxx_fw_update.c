...
int32_t Check_FW_Ver(void)
{
	...
	if (buf[1] == fw_entry->data[FW_BIN_VER_OFFSET])	//Not update firmware if ic firmware version equals to file firmware version
		return 1;
	else	//Update firmware if ic firmware version doe not equal to file firmware version
		return 0;
}
