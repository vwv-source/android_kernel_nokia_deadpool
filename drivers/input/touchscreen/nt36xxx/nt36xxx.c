 ...
 *
 * $Revision: 31202 $
 * $Date: 2018-07-23 10:56:09 +0800 (?±ä?, 23 ä¸ƒæ? 2018) $
 *
...
extern char fih_touch[32];	//SW4-HL-Touch-ImplementVirtualFileNode-00+_20180627
uint8_t bTouchIsAwake = 0;	//SW4-HL-Touch-UsbPlugInOut-00*_20181029
...
info_retry:
	...
	if ((buf[1] + buf[2]) != 0xFF) {
		...
		//[FIH], Print FW version
		NVT_ERR("IC Read fw_ver=%02d, x_num=%d, y_num=%d, \
			abs_x_max=%d, abs_y_max=%d, max_button_num=%d!\n",
			ts->fw_ver, ts->x_num, ts->y_num,
			ts->abs_x_max, ts->abs_y_max, ts->max_button_num);
		ret = 0;
...
int touch_double_tap_write_nvt(int enable)
{
	...
	if (!gLcmOnOff)
	{
		pr_debug("[HL]%s: LCM is OFF! Not Allow To Modify Double Tap Value!\n", __func__);
		return -1;
	}
	else
	{
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EARLY_EVENT_BLANK) {
		...
		if (*blank == FB_BLANK_POWERDOWN) {
			pr_debug("[HL]%s, %d: blank == FB_BLANK_POWERDOWN\n", __func__, __LINE__);
			nvt_ts_suspend(&ts->client->dev);
			gLcmOnOff = 0;	//SW4-HL-TP-B2N-NT36672-DoubleTap-00*_20180302
			pr_debug("[HL]%s, %d: tpd_usb_plugin(bUsbState) +++", __func__, __LINE__);
			tpd_usb_plugin(bUsbState); //SW4-HL-Touch-UsbPlugInOut-00+{_20181029
			pr_debug("[HL]%s, %d: tpd_usb_plugin(bUsbState) ---", __func__, __LINE__);
		}
		...
		if ((*blank == FB_BLANK_UNBLANK) || (*blank == FB_BLANK_NORMAL)) {
			...
			{
				pr_debug("[HL]%s, %d: blank == FB_BLANK_UNBLANK\n", __func__, __LINE__);
			}
			else if (*blank == FB_BLANK_NORMAL)
			{
				pr_debug("[HL]%s, %d: blank == FB_BLANK_NORMAL\n", __func__, __LINE__);
			}
			
			nvt_ts_resume(&ts->client->dev);
			gLcmOnOff = 1;	//SW4-HL-TP-B2N-NT36672-DoubleTap-00*_20180302
			pr_debug("[HL]%s, %d: tpd_usb_plugin(bUsbState) +++", __func__, __LINE__);
			tpd_usb_plugin(bUsbState); //SW4-HL-Touch-UsbPlugInOut-00+{_20181029
			pr_debug("[HL]%s, %d: tpd_usb_plugin(bUsbState) ---", __func__, __LINE__);
		}
